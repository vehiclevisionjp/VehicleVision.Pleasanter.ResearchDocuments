...
```text
docs/research/{カテゴリ連番}-{カテゴリ}/{ドキュメント連番}-{内容を端的に表すケバブケース}.md
```
...
| ルール           | 説明                                                        |
| ---------------- | ----------------------------------------------------------- |
| カテゴリフォルダ | 0 埋め 2 桁連番付きの英語フォルダに格納する（`01-auth` 等） |
...
| ファイル名                             | 説明               |
| -------------------------------------- | ------------------ |
| `03-data-api/001-upsert.md`            | Upsert API の実装  |
| `02-session/002-session-management.md` | Session 管理の実装 |
| `01-auth/002-permission-hierarchy.md`  | 権限階層構造       |
