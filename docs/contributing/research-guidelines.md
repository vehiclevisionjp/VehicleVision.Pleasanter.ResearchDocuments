...
| `01-認証・権限`         | 認証・権限         |
| `02-セッション`         | セッション         |
| `03-データ操作・API`    | API・データ操作    |
| `04-外部連携・フィード` | 外部連携・フィード |
| `05-通知`               | 通知               |
| `06-一覧・ビュー`       | 一覧・ビュー       |
| `07-編集画面・モーダル` | 編集画面・モーダル |
| `08-Markdown`           | Markdown           |
| `09-フロントエンド基盤` | フロントエンド基盤 |
| `10-ServerScript`       | ServerScript       |
| `11-CodeDefiner`        | CodeDefiner        |
| `12-データベース`       | データベース       |
| `13-タイムゾーン`       | タイムゾーン       |
| `14-セキュリティ`       | セキュリティ       |
| `15-拡張機能・多言語`   | 拡張機能・多言語   |
| `16-新SiteType`         | 新 SiteType        |
| `17-検索`               | 検索               |
| `18-管理画面`           | 管理画面           |
| `19-開発環境`           | 開発環境           |
| `20-開発ワークフロー`   | 開発ワークフロー   |

既存のカテゴリに該当しないドキュメントを追加する場合は、新しいカテゴリフォルダを作成すること。連番は既存の最大値 + 1 とし、上記のカテゴリ一覧にも追記すること。
