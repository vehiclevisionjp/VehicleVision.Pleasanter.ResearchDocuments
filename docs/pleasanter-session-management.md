...
- [プリザンター Session 管理の実装調査](#プリザンター-session-管理の実装調査)
  - [調査情報](#調査情報)
  - [調査目的](#調査目的)
  ...
  - [UserAreaセッション](#userareaセッション)
    ...
    - [UserArea CRUDタイミング](#userarea-crudタイミング)
      - [Create/Update（作成・更新）](#createupdate作成更新)
      - [Read（読み取り）](#read読み取り-1)
      - [Delete（削除）](#delete削除-1)
    - [UserArea CRUDフロー図](#userarea-crudフロー図)
  - [セッションライフサイクル管理](#セッションライフサイクル管理)
    - [RDB方式のライフサイクル](#rdb方式のライフサイクル)
      - [1. ReadOnceセッションの削除（Get時）](#1-readonceセッションの削除get時)
      - [2. 古いセッションの一括削除（DeleteOldSessions）](#2-古いセッションの一括削除deleteoldsessions)
    - [KVS（Redis）方式のライフサイクル](#kvsredis方式のライフサイクル)
      - [1. TTL（有効期限）の設定](#1-ttl有効期限の設定)
      - [2. ReadOnceセッションの削除](#2-readonceセッションの削除)
      - [3. 明示的な削除](#3-明示的な削除)
      - [4. 古いセッションの一括削除（注意点）](#4-古いセッションの一括削除注意点)
  ...
  - [技術的詳細分析](#技術的詳細分析)
    - [KVSのアンダースコア解析問題](#kvsのアンダースコア解析問題)
      - [問題の原因](#問題の原因)
      - [問題パターンの具体例](#問題パターンの具体例)
      - [問題の影響範囲](#問題の影響範囲)
      - [結論](#結論-1)
    - [KVS Remove非対応によるデータ残留リスク](#kvs-remove非対応によるデータ残留リスク)
      - [現在の実装状況](#現在の実装状況)
      - [Remove()が呼び出されるシナリオ](#removeが呼び出されるシナリオ)
      - [個別対応が行われているケース](#個別対応が行われているケース)
      - [個別対応がないケース（残留リスクあり）](#個別対応がないケース残留リスクあり)
      - [リスク評価まとめ](#リスク評価まとめ)
      - [具体的なリスクシナリオ](#具体的なリスクシナリオ)
      - [推奨される対策](#推奨される対策)
    - [KVS RemoveとTTLの共存設計](#kvs-removeとttlの共存設計)
      - [Redisの基本動作](#redisの基本動作)
      - [共存パターンの比較](#共存パターンの比較)
      - [推奨実装: パターン1（HDELのみ）](#推奨実装-パターン1hdelのみ)
      - [代替実装: パターン3（HDEL + 空チェック）](#代替実装-パターン3hdel--空チェック)
      - [ReadOnceとの整合性](#readonceとの整合性)
      - [TTL更新タイミングの考慮](#ttl更新タイミングの考慮)
      - [まとめ](#まとめ-1)
    - [推奨事項](#推奨事項)
