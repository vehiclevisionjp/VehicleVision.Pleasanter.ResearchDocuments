...
プリザンターの認証基盤（Cookie 認証・API キー認証・Bearer ヘッダー認証・権限チェック）の内部実装を調査した。
...
- [Bearer ヘッダーによる API キー認証](#bearer-ヘッダーによる-api-キー認証)
    - [概要](#概要)
    - [BinariesController でのファイルアップロード](#binariescontroller-でのファイルアップロード)
    - [Bearer ヘッダーの中身](#bearer-ヘッダーの中身)
    - [LINE 通知での Bearer ヘッダー（外部 API 呼出）](#line-通知での-bearer-ヘッダー外部-api-呼出)
    - [JWT Bearer 認証との違いまとめ](#jwt-bearer-認証との違いまとめ)
- [権限チェック](#権限チェック)
...
- Bearer ヘッダーによる API キー送信方式を把握する
...
## Bearer ヘッダーによる API キー認証

### 概要

プリザンターは **JWT Bearer 認証ミドルウェアを使用していない**。
`Startup.cs` に `AddJwtBearer()` は登録されておらず、
認証スキームは `CookieAuthenticationDefaults.AuthenticationScheme` のみである。

ただし、`Authorization: Bearer` ヘッダーを**API キーの送信手段**として
利用する箇所が存在する。
これはファイルアップロードなど `multipart/form-data` を使用する
エンドポイントで、リクエストボディに JSON を含められないための代替手段である。

### BinariesController でのファイルアップロード

ファイルアップロード API では、`Authorization: Bearer <APIキー>` ヘッダーから
API キーを取得し、通常の API キー認証フローに合流する。
...
sequenceDiagram
    ...
    Client->>Filter: POST /api/Binaries/Upload<br>Authorization: Bearer <APIキー><br>Content-Type: multipart/form-data
    Note right of Filter: IP制限等のチェック
    Filter->>Ctrl: アクション実行
    Ctrl->>Ctrl: AuthorizationHeaderValue()<br>— Bearer ヘッダーから APIキー抽出
    Ctrl->>Ctx: new Context(apiRequestBody: {ApiKey: ...}, api: true)
    Ctx->>DB: ApiKey で Users テーブルを検索
    DB-->>Ctx: UserModel
    Ctx->>Ctx: Authenticated 判定
    Ctrl->>Ctrl: context.Authenticated で認証チェック
```

**API キー抽出処理**:

```csharp
// Controllers/Api/BinariesController.cs L281-290
private string AuthorizationHeaderValue()
{
    var authHeader = (string)Request.Headers["Authorization"];
    if (authHeader != null
        && authHeader.ToLower().StartsWith("bearer"))
    {
        return authHeader
...
**Context 生成時の API キー注入**:

```csharp
// Controllers/Api/BinariesController.cs L93-99
var context = new Context(
...
抽出した API キーを JSON 文字列 `{"ApiKey": "..."}` に変換して
`apiRequestBody` パラメータに渡す。
これにより通常の API キー認証フロー
（`Context.SetUserProperties()` → `ApiKey` で Users 検索）に合流する。

### Bearer ヘッダーの中身

Bearer トークンの中身は **JWT ではなく、プリザンターの API キー**
（GUID を SHA-512 でハッシュ化した文字列）である。

```text
# リクエスト例（ファイルアップロード）
POST /api/Binaries/1234/Upload HTTP/1.1
Authorization: Bearer <SHA-512ハッシュのAPIキー>
Content-Type: multipart/form-data; boundary=...
```

| 項目           | 内容                                      |
| -------------- | ----------------------------------------- |
| ヘッダー形式   | `Authorization: Bearer <APIキー>`         |
| トークンの中身 | プリザンター API キー（SHA-512 ハッシュ） |
| JWT            | **使用していない**                        |
| 用途           | `multipart/form-data` での API 認証       |
| 認証フロー     | 通常の API キー認証と同一                 |

### LINE 通知での Bearer ヘッダー（外部 API 呼出）

プリザンターは LINE Messaging API への通知送信時にも
`Authorization: Bearer` ヘッダーを使用するが、
これは**外部サービスへの発信用**であり、
プリザンター自体の認証基盤とは無関係である。

```csharp
// Libraries/DataSources/Line.cs L48-51
...
### JWT Bearer 認証との違いまとめ

| 項目                             | プリザンターの実装                | 一般的な JWT Bearer 認証        |
| -------------------------------- | --------------------------------- | ------------------------------- |
| ASP.NET Core ミドルウェア        | **未登録**（`AddJwtBearer` なし） | `AddJwtBearer()` で登録         |
| トークン形式                     | API キー（SHA-512 文字列）        | JWT（ヘッダー.ペイロード.署名） |
| トークン検証                     | DB の `Users.ApiKey` 照合         | 署名検証 + 有効期限チェック     |
| トークン発行                     | ユーザー管理画面から生成          | 認証エンドポイントで発行        |
| 有効期限                         | なし（手動で再生成）              | `exp` クレームで自動失効        |
| `Authorization` ヘッダー利用箇所 | ファイルアップロードのみ          | 全 API エンドポイント           |
...
| Bearer ヘッダー認証   | JWT ではなく API キーを Bearer 形式で送信。ファイルアップロード API でのみ使用                    |
...
| `Implem.Pleasanter/Controllers/Api/BinariesController.cs`           | Bearer ヘッダーでの API キー取得 |
| `Implem.Pleasanter/Libraries/DataSources/Line.cs`                   | LINE 通知（Bearer 送信）         |
