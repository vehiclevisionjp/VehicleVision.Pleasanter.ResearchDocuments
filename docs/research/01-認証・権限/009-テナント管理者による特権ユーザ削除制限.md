...
- [パラメータによる制御機能の追加](#パラメータによる制御機能の追加)
    - [要件](#要件)
    - [パラメータ設計](#パラメータ設計)
    - [実装案（パラメータ制御版）](#実装案パラメータ制御版)
    - [パラメータ制御のメリット](#パラメータ制御のメリット)
    - [デフォルト値の推奨設定](#デフォルト値の推奨設定)
    - [注意事項（パラメータ制御版）](#注意事項パラメータ制御版)
- [注意事項](#注意事項)
    - [SiteInfo.User の取得について](#siteinfouser-の取得について)
    - [UI と API の両方で機能させる](#ui-と-api-の両方で機能させる)
    - [既存のセルフチェック機能](#既存のセルフチェック機能)
- [実装手順](#実装手順)
    - [方式 1: ビットマスク/Enum 方式（推奨）](#方式-1-ビットマスクenum-方式推奨)
    - [方式 2: Boolean 方式](#方式-2-boolean-方式)
    - [共通ステップ: エラーメッセージの追加（オプション）](#共通ステップ-エラーメッセージの追加オプション)
    - [ステップ 5: テスト](#ステップ-5-テスト)
- [まとめ](#まとめ)
    - [パラメータ制御のメリット](#パラメータ制御のメリット-1)
- [関連ソースコード](#関連ソースコード)
...
特権ユーザの操作制限を柔軟に設定できるよう、パラメータで制御可能にする:

1. **削除制限**: 特権ユーザの削除を特権ユーザのみに制限する
2. **編集制限**: 特権ユーザの編集を特権ユーザのみに制限する

### パラメータ設計

#### 方式 1: ビットマスク/Enum 方式（推奨）

`Implem.ParameterAccessor/Parts/Security.cs` に Enum 定義とパラメータを追加:
...
- `"Delete"`: 削除のみ制限
- `"Update"`: 編集のみ制限
- `"Delete, Update"`: 削除と編集の両方を制限

**メリット**:

- 複数の制限を1つのパラメータで管理できる
- 将来的に新しい制限（例: Read）を追加しやすい
- ビット演算で効率的にチェック可能
- JSON での設定が直感的

#### 方式 2: Boolean 方式

`Implem.ParameterAccessor/Parts/Security.cs` に Boolean パラメータを追加:
...
```json
{
    "Security": {
        "PrivilegedUsers": ["admin", "system"],
        "RestrictPrivilegedUserDeletion": true,
        "RestrictPrivilegedUserUpdate": true
    }
}
```
...
- 制限の種類が増えるとパラメータが増加する

### 実装案（パラメータ制御版）

#### 方式 1: ビットマスク/Enum 方式の実装
...
case "users":
    ...
    // パラメータで制限が有効な場合のみチェック
    if (canDelete && Parameters.Security.RestrictPrivilegedUserDeletion)
    {
        var targetUser = SiteInfo.User(
            context: context,
            userId: context.Id);
...
`Permissions.cs` の `CanUpdate` メソッドにも同様の制限を追加:

**現在のコード** (`Permissions.cs` L556-559):
...
case "users":
    ...
    // パラメータで制限が有効な場合のみチェック
    if (canUpdate && Parameters.Security.RestrictPrivilegedUserUpdate)
    {
        var targetUser = SiteInfo.User(
            context: context,
            userId: context.Id);
...
| メリット           | 説明                                                               |
| ------------------ | ------------------------------------------------------------------ |
| 柔軟な運用         | 環境や要件に応じて制限の有効/無効を切り替え可能                    |
| 段階的な導入       | まずは削除制限のみ有効化し、運用を確認してから編集制限を追加できる |
| 後方互換性の維持   | デフォルトを `false` にすることで既存環境への影響を最小化          |
| 明確な制御ポイント | パラメータファイルで一元管理できるため、設定変更が容易             |
| テストの容易さ     | パラメータを変えるだけで異なる動作を検証可能                       |

### デフォルト値の推奨設定

セキュリティを重視する場合は、以下のデフォルト値を推奨:
...
{
    "Security": {
        "RestrictPrivilegedUsers": "Delete"
    }
}
```
...
```json
{
    "Security": {
        "RestrictPrivilegedUserDeletion": true,
        "RestrictPrivilegedUserUpdate": false
    }
}
```
...
- **削除制限**: 特権ユーザの削除は取り返しがつかないため、デフォルトで有効化
- **編集制限**: 一般的なユースケースでは過度な制限となる可能性があるため、デフォルトは無効化し、必要に応じて有効化

### 注意事項（パラメータ制御版）

1. **自己編集の考慮**: `context.UserId == context.Id` の場合は、特権ユーザでなくても自分自身の編集は許可される（既存動作を維持）
2. **EnableManageTenant の影響**: `EnableManageTenant` ユーザは、パラメータで制限が有効な場合、特権ユーザの編集ができなくなる
3. **パフォーマンス**: `SiteInfo.User()` の呼び出しは各リクエストで発生するが、キャッシュから取得されるため影響は軽微
...
#### ステップ 1: Enum 定義と Security パラメータへの追加

`Implem.ParameterAccessor/Parts/Security.cs` に Enum 定義と新規プロパティを追加:
...
`App_Data/Parameters/Security.json` に設定値を追加:
...
#### ステップ 2: CanDelete メソッドの改修

`Implem.Pleasanter/Libraries/Security/Permissions.cs` の `CanDelete` メソッドを改修:
...
#### ステップ 3: CanUpdate メソッドの改修

`Implem.Pleasanter/Libraries/Security/Permissions.cs` の `CanUpdate` メソッドを改修:
...
#### ステップ 1: Security パラメータへの追加

`Implem.ParameterAccessor/Parts/Security.cs` に新規プロパティを追加:
...
public enum Types
{
    ...
    CannotUpdatePrivilegedUser = XXX,
}
```
...
```json
{
    ...
    "CannotUpdatePrivilegedUser": {
        "Ja": "特権ユーザは編集できません。",
        "En": "Cannot update a privileged user."
    }
}
```
...
#### 削除機能のテスト（RestrictPrivilegedUserDeletion = true）
...
#### 編集機能のテスト（RestrictPrivilegedUserUpdate = true）

1. テナント管理者が一般ユーザを編集できる（既存機能）
2. テナント管理者が特権ユーザを編集しようとするとエラーになる（新機能）
3. 特権ユーザが別の特権ユーザを編集できる（新機能）
4. 一般ユーザが自分自身のプロファイルを編集できる（既存機能）
5. API 経由でも同じ制限が適用される（新機能）

#### パラメータ無効時のテスト

1. RestrictPrivilegedUserDeletion = false の場合、テナント管理者が特権ユーザを削除できる
2. RestrictPrivilegedUserUpdate = false の場合、テナント管理者が特権ユーザを編集できる

## まとめ

テナント管理者による特権ユーザの削除・編集を制御するには、以下の改修を行う:

1. `Parameters.Security` に `RestrictPrivilegedUserDeletion` と `RestrictPrivilegedUserUpdate` パラメータを追加
2. `Permissions.CanDelete` メソッドでパラメータに基づいて削除対象ユーザが特権ユーザかどうかをチェック
3. `Permissions.CanUpdate` メソッドでパラメータに基づいて編集対象ユーザが特権ユーザかどうかをチェック
4. 特権ユーザの操作は特権ユーザのみ許可する
5. エラーメッセージを追加して、操作が拒否された理由を明確にする

### パラメータ制御のメリット

- **柔軟な運用**: 環境や要件に応じて制限の有効/無効を切り替え可能
- **段階的な導入**: まずは削除制限のみ有効化し、運用を確認してから編集制限を追加できる
- **後方互換性**: デフォルトを `false` にすることで既存環境への影響を最小化

この改修により、特権ユーザの誤操作を防止し、セキュリティを向上させることができる。
