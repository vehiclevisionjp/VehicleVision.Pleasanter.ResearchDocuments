...
| SAML             | 外部 IdP         | ○                | 組込（AllowOriginalLogin） | `"SAML"`             |
...
> 2 段階認証・補助認証（TrustedProxy / Passkey / Extension）・認証チケットについては [2 段階認証・補助認証・パスキー](011-2段階認証・補助認証・パスキー.md) を参照。
...
- [2 段階認証・補助認証・パスキー](011-2段階認証・補助認証・パスキー.md) — 2FA（メール/TOTP）、TrustedProxy、Passkey（WebAuthn）、Extension の実装詳細
