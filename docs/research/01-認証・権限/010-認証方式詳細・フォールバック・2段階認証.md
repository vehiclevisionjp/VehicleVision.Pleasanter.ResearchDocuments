...
`Provider = "Windows"` の場合、LDAP 認証と同じコードパスで処理される。
...
> `Implem.Pleasanter/Libraries/Security/Authentications.cs:69-72`

`context.AuthenticationsWindows()` は IIS の Windows 認証モジュールが有効な場合に `true` を返す。
...
| `"Windows"`          | LDAP（AD）  | なし                              | LDAP 失敗 = 認証失敗                 |
