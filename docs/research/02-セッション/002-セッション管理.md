...
| 項目                   | 評価 | 概要                                                              |
| ---------------------- | ---- | ----------------------------------------------------------------- |
| **RDB/KVS二重構造**    | 可   | RDB（デフォルト）とRedis（オプション）の2方式をサポート           |
| **KVS拡張可能性**      | 不可 | 低（Redisへの直接依存、インターフェース抽象化なし）               |
| **UserAreaのKVS対応**  | 不可 | 非対応（常にRDBに保存）                                           |
| **自動クリーンアップ** | 注意 | Webリクエスト時のみ発火（バックグラウンドタスクでは実行されない） |
| **Remove()のKVS対応**  | 不可 | 非対応（個別キー削除時、RDBのみ削除されRedisにデータが残る）      |
...
1. **Page識別の差異**
    ...
    注意 キー名にアンダースコアを含む場合、Page解析が誤動作する可能性（詳細は[KVSのアンダースコア解析問題](#kvsのアンダースコア解析問題)を参照）
...
2. **ReadOnce管理の差異**
    ...
    注意 ReadOnce削除時、RDBは行単位、Redisはキー全体削除
...
| **Delete (個別キー)**     | `PhysicalDeleteSessions`               | **RDBのみ**（Redis削除なし）注意       |
...
flowchart TD
    ...
    subgraph "Get操作"
        G1{UseKeyValueStore?}
        G1 -->|true| G2[Redis: HashGetAll<br/>注意 UserAreaは取得不可]
...
| リクエスト種別          | 発火有無 | 理由                                               |
| ----------------------- | -------- | -------------------------------------------------- |
| 通常のWebリクエスト     | 可       | `sessionData: true` がデフォルト                   |
| APIリクエスト(認証済み) | 可       | 同上                                               |
| バックグラウンド処理    | 不可     | `sessionData: false` または tenantIdコンストラクタ |
| 認証・フィルター処理    | 不可     | `sessionData: false`                               |
| 初期化処理              | 不可     | tenantIdコンストラクタ                             |
...
**注意**: `Remove()` は常にRDBに対してのみ削除を実行し、KVS（Redis）側は削除されません。
...
| プラグイン機構           | なし                            | 低         |
| 処理の局所化             | SessionUtilitiesに集約          | 注意 中    |
...
| ------ | ------------ | ----------- | ---------------------- | -------------------------------- | ------- | ------------- | ------------ | ---------- |
| 1      | `ViewMode`   | (なし)      | `ViewMode`             | `["ViewMode"]`                   | 可      | -             | `ViewMode`   | 正常       |
| 2      | `ViewMode`   | `Items/100` | `ViewMode_Items/100`   | `["ViewMode", "Items/100"]`      | 可      | 可            | `ViewMode`   | 正常       |
| 3      | `User_Theme` | (なし)      | `User_Theme`           | `["User", "Theme"]`              | 不可    | "Theme"と誤認 | `User_Theme` | **誤動作** |
| 4      | `User_Theme` | `Items/100` | `User_Theme_Items/100` | `["User", "Theme", "Items/100"]` | 不可    | "Theme"と誤認 | `User_Theme` | **誤動作** |
| 5      | `My_Key`     | `Page_123`  | `My_Key_Page_123`      | `["My", "Key", "Page", "123"]`   | 不可    | "Key"と誤認   | `My_Key`     | **誤動作** |
...
| `UserUtilities.SwitchUser`             | `SwitchLoginId`        | 切り替え失敗時のクリーンアップ | 注意 残留リスクあり |
| `SessionExclusive.Clear`               | `TableExclusive_*`等   | 排他制御ロック解放             | 注意 残留リスクあり |
| `BinaryUtilities`                      | `TempFile_{guid}`      | 一時ファイル管理               | 注意 残留リスクあり |
| `BinariesController`                   | セッションGUID         | アップロードエラー時           | 注意 残留リスクあり |
| `AspNetCoreKeyManagementXmlRepository` | XMLリポジトリキー      | 暗号化キー管理                 | 注意 残留リスクあり |
