...
| 項目                 | 内容                                                                                                                    |
| -------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| レースコンディション | Upsert API はレコード検索と作成・更新の間にロック機構がなく、同一 Keys での並列呼び出しでレースコンディションが発生する |
| 原因                 | `AccessStatus` による存在確認とその後の Create/Update がアトミックに実行されない                                        |
| プリザンター側対策   | 現時点で未実装（トランザクション分離レベルの変更やアドバイザリーロック等は導入されていない）                            |
| クライアント側対策   | SemaphoreSlim による排他制御、リトライ＋指数バックオフでの回避が有効                                                    |
