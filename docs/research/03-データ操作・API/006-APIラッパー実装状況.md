...
- [主な差違と注意点](#主な差違と注意点)
    ...
    - [レイヤー間の取得フィールド差違](#レイヤー間の取得フィールド差違)
- [関連ソースコード](#関連ソースコード)
- [結論](#結論)
...
| ------- | ------------------ | -------------------- | -------------------------------------------------------------------- |
| Get     | なし               | なし（直接対応なし） | サーバースクリプトでは `context.UserData` でリクエスト内データ共有可 |
| Set     | なし               | なし（直接対応なし） | 同上                                                                 |
| Delete  | なし               | なし（直接対応なし） | 同上                                                                 |

`context.UserData` は `ExpandoObject` 型の動的プロパティで、同一リクエスト内の複数のサーバースクリプト間でデータを共有できる。
Sessions API（`SavePerUser: true`）がリクエストをまたいでユーザ単位にデータを永続化するのに対し、
`context.UserData` はリクエストスコープ内での一時的なデータ共有に使用する。
...
| 機能                                  | スクリプト | サーバースクリプト                                 | 補足                                                 |
| ------------------------------------- | ---------- | -------------------------------------------------- | ---------------------------------------------------- |
| リクエスト内データ共有                | なし       | `context.UserData`                                 | 同一リクエスト内のサーバースクリプト間でデータを共有 |
...
| Sessions（Get/Set/Delete）       | セッション操作は Web API のみ（サーバースクリプトは `context.UserData` でリクエスト内データ共有可） |
...
同じ「ユーザ情報取得」でも、使用するレイヤーによって取得できるフィールドが大きく異なる。

#### Users

| フィールド                              | Web API (`api/users/get`)              | スクリプト (`$p.userId()` 等) | サーバースクリプト (`users.Get(id)`) | サーバースクリプト (`context`) |
| --------------------------------------- | -------------------------------------- | ----------------------------- | ------------------------------------ | ------------------------------ |
| TenantId                                | 対応                                   | -                             | -                                    | 対応                           |
| UserId                                  | 対応                                   | `$p.userId()`                 | 対応                                 | 対応                           |
| LoginId                                 | 対応                                   | `$p.loginId()`                | 対応                                 | 対応                           |
| Name                                    | 対応                                   | `$p.userName()`               | 対応                                 | -                              |
| UserCode                                | 対応                                   | -                             | 対応                                 | -                              |
| DeptId                                  | 対応                                   | `$p.deptId()`                 | 対応                                 | 対応                           |
| TenantManager                           | 対応                                   | -                             | 対応                                 | -                              |
| ServiceManager                          | 対応                                   | -                             | 対応                                 | -                              |
| Disabled                                | 対応                                   | -                             | 対応                                 | -                              |
| MailAddresses                           | 対応（要 `ApiGetMailAddresses: true`） | -                             | -                                    | -                              |
| Password / LastName / FirstName 等      | 対応                                   | -                             | -                                    | -                              |
| Birthday / Gender / Language / TimeZone | 対応                                   | -                             | -                                    | Language / TimeZoneInfo        |
| Body / LastLoginTime 等                 | 対応                                   | -                             | -                                    | -                              |
| GroupIds                                | -                                      | `$p.groupIds()`               | -                                    | Groups                         |
| Theme                                   | 対応                                   | `$p.theme()`                  | -                                    | -                              |
| HasPrivilege                            | -                                      | -                             | -                                    | 対応                           |

- Web API は `UserApiModel` の全 40 以上のフィールドを返却可能（サイト設定の読み取り権限に従う）
- `MailAddresses` は Web API でのみ取得可能で、リクエストに `ApiGetMailAddresses: true` の指定が必要
- スクリプトの `$p.userId()` 等は HTML 隠しフィールドからの取得で、現在のログインユーザの基本情報のみ
- サーバースクリプトの `users.Get(id)` は `SiteInfo` キャッシュからの取得で、9 フィールドに限定
- サーバースクリプトの `context` は現在のリクエストコンテキストで、ユーザ基本情報に加えリクエスト情報を含む

#### Depts

| フィールド | Web API (`api/depts/get`) | スクリプト (`$p.apiDeptsGet`) | サーバースクリプト (`depts.Get(id)`) |
| ---------- | ------------------------- | ----------------------------- | ------------------------------------ |
| TenantId   | 対応                      | 対応（API 経由）              | -                                    |
| DeptId     | 対応                      | 対応（API 経由）              | 対応                                 |
| DeptCode   | 対応                      | 対応（API 経由）              | 対応                                 |
| DeptName   | 対応                      | 対応（API 経由）              | 対応                                 |
| Body       | 対応                      | 対応（API 経由）              | -                                    |
| Disabled   | 対応                      | 対応（API 経由）              | -                                    |
| Ver 等     | 対応                      | 対応（API 経由）              | -                                    |
| Comments   | 対応                      | 対応（API 経由）              | -                                    |
| GetMembers | -                         | -                             | 対応（メソッド）                     |

- スクリプトの `$p.apiDeptsGet` は Web API を呼び出すため、Web API と同等のフィールドを取得可能
- サーバースクリプトの `depts.Get(id)` は `SiteInfo` キャッシュからの取得で 4 フィールドに限定
- `GetMembers()` メソッドはサーバースクリプトのみ提供

#### Groups

| フィールド    | Web API (`api/groups/get`) | スクリプト (`$p.apiGroupsGet`) | サーバースクリプト (`groups.Get(id)`) |
| ------------- | -------------------------- | ------------------------------ | ------------------------------------- |
| TenantId      | 対応                       | 対応（API 経由）               | -                                     |
| GroupId       | 対応                       | 対応（API 経由）               | 対応                                  |
| GroupName     | 対応                       | 対応（API 経由）               | 対応                                  |
| Body          | 対応                       | 対応（API 経由）               | 対応                                  |
| Disabled      | 対応                       | 対応（API 経由）               | 対応                                  |
| Ver 等        | 対応                       | 対応（API 経由）               | -                                     |
| LdapSync 等   | 対応                       | 対応（API 経由）               | -                                     |
| GroupMembers  | 対応                       | 対応（API 経由）               | 対応（`GetMembers` メソッド）         |
| GroupChildren | 対応                       | 対応（API 経由）               | 対応（`GetChildren` メソッド）        |
| Comments      | 対応                       | 対応（API 経由）               | -                                     |

- スクリプトの `$p.apiGroupsGet` は Web API を呼び出すため、Web API と同等のフィールドを取得可能
- サーバースクリプトの `groups.Get(id)` は `SiteInfo` キャッシュから 4 フィールドのみ取得
- メンバー・子グループ情報は Web API ではレスポンスに含まれ、サーバースクリプトでは専用メソッドで取得
...
| `Implem.Pleasanter/Models/Users/UserApiModel.cs`                             | Users API レスポンスモデル                   |
| `Implem.Pleasanter/Libraries/ServerScripts/ServerScriptModelUserModel.cs`    | サーバースクリプト users.Get 戻り値モデル    |
| `Implem.Pleasanter/Libraries/ServerScripts/ServerScriptModelContext.cs`      | サーバースクリプト context オブジェクト      |
| `Implem.PleasanterFrontend/wwwroot/src/scripts/generals/_elements.js`        | スクリプト `$p.userId` 等の定義              |
