...
| 箇所                           | 呼び出し経路                                               | 展開 |
| ------------------------------ | ---------------------------------------------------------- | ---- |
| 件名（Subject）                | `ReplacedDisplayValues` → `ReplacedContextValues`          | 可   |
| 本文（NoticeBody: 非カラム行） | `NoticeBody` → `ReplacedContextValues` (column == null 時) | 可   |
| 本文（NoticeBody: カラム行）   | `NoticeBody` → `ToNotice` → `DisplayText`                  | 不可 |
| プロセス通知 Subject           | `ReplacedDisplayValues` → `ReplacedContextValues`          | 可   |
| プロセス通知 Body              | `ReplacedDisplayValues` → `ReplacedContextValues`          | 可   |
| 通知 Prefix                    | `Notification.Send` で `Prefix + title` として直接結合     | 不可 |
| 宛先アドレス                   | `IncludedColumns` → `ReplacedAddress`                      | 不可 |
...
flowchart TD
    ...
    H --> K["ReplacedContextValues 適用 可"]
    J --> L["ToNotice → DisplayText 不可"]
    K --> M["body に追加"]
    L --> M
...
| ------------------------------------- | ---------------------- | ---------------------- | --------------------------------------------- |
| `{Url}`                               | 不可（キー未引用）     | 例外 → `null`          | ReplacedContextValues 可                      |
| `{UserName}<{MailAddress}>`           | 不可                   | 例外 → `null`          | ReplacedContextValues 可                      |
| `{Url}{UserName}`                     | 不可                   | 例外 → `null`          | ReplacedContextValues 可                      |
| `{}`                                  | 可（空オブジェクト）   | 成功（全 null）        | ReplacedContextValues（マッチなし）注意       |
| `{}{}`                                | 不可（追加コンテンツ） | 例外 → `null`          | ReplacedContextValues（マッチなし）注意       |
| `{"Name":"[Title]"}`                  | 可                     | 成功（Name="[Title]"） | カラム分岐                                    |
| `{"Name":"[Title]"}{Url}`             | 不可（13.0.4）         | 例外 → `null`          | ReplacedContextValues 可（JSON リテラル混入） |
| `{"Name":"[Title]","Prefix":"{Url}"}` | 可                     | 成功（Prefix="{Url}"） | カラム分岐（Prefix 未展開）不可               |
| 空文字列                              | 不可                   | 例外 → `null`          | ReplacedContextValues（空行出力）可           |
...
| ケース | 入力例                                   | 修正後の出力                                         | 解決 |
| ------ | ---------------------------------------- | ---------------------------------------------------- | ---- |
| P1     | `{"Name":"[Title]","Prefix":"{Url} — "}` | `https://example.com/items/123 — タイトル : 値`      | 可   |
| P2     | `{"Name":"[Title]"}{Url}`                | `{"Name":"[Title]"}https://...`（JSON リテラル残留） | 注意 |
...
| ------ | ---------------------------------------- | ---------------------------------------------------- | ---- |
| P1     | `{"Name":"[Title]","Prefix":"{Url} — "}` | `https://example.com/items/123 — タイトル : 値`      | 可   |
| P2     | `{"Name":"[Title]"}{Url}`                | `{"Name":"[Title]"}https://...`（JSON リテラル残留） | 不可 |
...
| ケース | 入力例                                   | 修正後の出力                                   | 解決 |
| ------ | ---------------------------------------- | ---------------------------------------------- | ---- |
| P1     | `{"Name":"[Title]","Prefix":"{Url} — "}` | `{Url} — タイトル : 値`（Prefix 未展開のまま） | 不可 |
| P2     | `{"Name":"[Title]"}{Url}`                | `タイトル : 値https://example.com/items/123`   | 可   |
