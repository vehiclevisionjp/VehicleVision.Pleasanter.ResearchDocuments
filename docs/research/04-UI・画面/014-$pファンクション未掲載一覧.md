...
未掲載ファンクションの各カテゴリが「何のために使われているか」を簡潔にまとめた。

| カテゴリ                   | 件数 | 使用用途                                                                                                     | ユーザスクリプトでの利用 |
| -------------------------- | ---: | ------------------------------------------------------------------------------------------------------------ | :----------------------: |
| 内部イベントディスパッチ   |   10 | `$p.send()` 内部で呼ばれるイベントパイプライン。`$p.events` に登録したハンドラを適切なタイミングで発火する   | 不要（`$p.events` 経由） |
| AJAX・通信関連             |    6 | ファイルアップロード、CAPTCHA、API URL生成など `$p.ajax()` / `$p.send()` の内部補助                          |         基本不要         |
| フォーム・データ管理       |    8 | `$p.data` への格納処理や複数選択コントロールの内部操作。`$p.set()` / `$p.setData()` の下位処理               |         基本不要         |
| ローディング・スクロール   |    8 | ローディングスピナーの表示/非表示、スクロール位置の保存/復元、ページネーション制御                           |         一部有用         |
| 一覧画面・グリッド操作     |    7 | 一覧画面のインライン編集、行コピー、新規行追加など。編集画面をモーダルで開く処理も含む                       |         一部有用         |
| ナビゲーション・ページ遷移 |    5 | SSO ログイン、レコード移動、パスワード変更画面への遷移など                                                   |         基本不要         |
| ダイアログ管理             |    3 | 汎用ダイアログのクリア、HTMLダイアログ、レスポンシブメニューの表示                                           |         基本不要         |
| サイト設定ダイアログ       |   52 | テーブル管理画面の各種設定ダイアログ（カラム設定、通知、プロセス、スクリプト等）の開閉と値の反映             | 使用不可（管理画面専用） |
| サイト設定確認ダイアログ   |    4 | ダッシュボードパーツ追加時のサイト選択確認ダイアログ                                                         | 使用不可（管理画面専用） |
| 権限管理                   |   15 | サイト・レコードの権限ダイアログ操作。カラムアクセス制御の追加/削除/変更                                     | 使用不可（管理画面専用） |
| アクセス制御               |    8 | ビュー・エクスポート・プロセス・状態制御の各アクセス制御の追加/削除                                          | 使用不可（管理画面専用） |
| ダッシュボード             |    6 | ダッシュボード画面のレイアウト更新、パーツのアクセス制御、非同期読み込み、ページング                         |         基本不要         |
| カレンダー・ガント等       |   10 | カレンダー/ガント/バーンダウン/タイムシリーズ/クロス集計の描画・移動・エクスポート                           |         基本不要         |
| アナリティクス             |    2 | 分析パーツの設定ダイアログとチャート描画                                                                     |         基本不要         |
| レコード操作               |    8 | 新規作成、コピー、検索、一括更新、インポート/エクスポートのUI操作トリガー                                    |         一部有用         |
| メール・通知               |    5 | 送信メールダイアログの表示・初期化、メール送信、宛先追加                                                     |         基本不要         |
| インポート・エクスポート   |    8 | サイトパッケージのインポート/エクスポートダイアログの表示と実行                                              |         基本不要         |
| 多言語ラベル               |    4 | 多言語ラベルのインポート/エクスポートダイアログと実行処理                                                    |         基本不要         |
| テンプレート               |    9 | サイト新規作成時のテンプレート選択・プレビュー・インポート・編集                                             |         基本不要         |
| 関連カラム                 |    5 | 親子関係のあるドロップダウン（関連カラム）の初期化とビュー変更時の連動処理                                   |         基本不要         |
| カラム操作                 |    7 | サイト設定画面でのカラムの並び替え・追加・削除（ドラッグ&ドロップ操作の裏側）                                | 使用不可（管理画面専用） |
| UI制御・レスポンシブ       |    7 | レスポンシブ切替、jQuery UI 再適用、フォーカス制御、ページ変更監視、アンカー表示制御                         |         一部有用         |
| サイドメニュー             |    2 | 左サイドメニューの展開/折りたたみ                                                                            |         基本不要         |
| アナウンスメント           |    1 | 画面上部のお知らせバナーを閉じる                                                                             |         基本不要         |
| フィルタ・ビュー設定       |    5 | ビュー切替、ドロップダウン検索、計算方法変更、エクスポートID選択                                             |         基本不要         |
| 数値・日付範囲ダイアログ   |   10 | フィルタ条件設定時の数値範囲/日付範囲の入力ダイアログの表示・確定・クリア                                    |         基本不要         |
| グループ管理               |    6 | グループ編集画面でのメンバー/子グループの追加・削除・移動                                                    | 使用不可（管理画面専用） |
| 画像・動画・添付ファイル   |   13 | 画像/添付ファイルの削除・アップロード、カメラ撮影、動画再生。QRコード読取用のカメラ操作も含む                |         基本不要         |
| サイトメニュー             |    3 | フォルダ階層のサイトメニュー構築、スタートガイド表示、リンクダイアログ                                       |         基本不要         |
| バスケット                 |    1 | 選択候補をバスケット（タグ入力風UI）に追加する                                                               |         基本不要         |
| クリップボード・QRコード   |    2 | レコードの直接URLをクリップボードにコピー、QRコード表示                                                      |           有用           |
| プロセス実行               |    2 | ボタン操作によるプロセスの実行と入力検証ダイアログの設定                                                     |         基本不要         |
| パスワード                 |    2 | ランダムパスワードの生成と生成ボタンのUI制御                                                                 |         基本不要         |
| ユーティリティ             |    9 | スロットル/デバウンス、アクション名取得、自動ポストバック制御、カンバン/画像ライブラリ設定など雑多な補助関数 |         一部有用         |
| 内部状態プロパティ         |   10 | マウス座標、スクロール位置、モーダル状態、検索ワードなど内部状態を保持するプロパティ群                       |        参照のみ可        |
