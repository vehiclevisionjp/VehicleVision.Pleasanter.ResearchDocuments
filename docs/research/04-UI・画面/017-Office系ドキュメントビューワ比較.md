...
自社プロダクトへ組み込む場合、ライセンスによるソースコード公開義務の有無が重要な選定基準となる。以下に各ライセンスの分類を示す。

| 分類                   | ライセンス   |      コード公開義務      | 該当ライブラリ                               |
| ---------------------- | ------------ | :----------------------: | -------------------------------------------- |
| パーミッシブ（許容型） | MIT          |            -             | docx-preview / x-spreadsheet                 |
| パーミッシブ（許容型） | BSD-2-Clause |            -             | mammoth.js                                   |
| パーミッシブ（許容型） | Apache-2.0   |            -             | SheetJS / Univer（フロントエンド）           |
| 弱コピーレフト         | MPL-2.0      |    ファイル単位で必要    | Collabora Online                             |
| 弱コピーレフト         | LGPL         |   ライブラリ改変時のみ   | （該当なし）                                 |
| 強コピーレフト         | AGPL-3.0     | ネットワーク経由でも必要 | ONLYOFFICE                                   |
| プロプライエタリ       | 商用         |            -             | Microsoft Office Online / Google Docs Viewer |

> **パーミッシブライセンス（MIT / BSD / Apache-2.0）は、自社プロダクトのソースコード公開義務が発生しない**ため、商用プロダクトへの組み込みに最も適している。
>
> **AGPL-3.0 は、ネットワーク経由で利用するだけでもソースコード公開義務が発生する**ため、ONLYOFFICE Community Edition を自社 SaaS に組み込む場合は商用ライセンスの購入が必要となる。
>
> **MPL-2.0 は、MPL で配布されたファイルの改変部分のみ公開すればよい**（ファイルレベルコピーレフト）ため、自社コードとの分離が容易であれば比較的利用しやすい。
...
| ライセンス                | コード公開義務のないパーミッシブライセンス（MIT / BSD / Apache-2.0）のライブラリは docx-preview・mammoth.js・SheetJS・x-spreadsheet に限られる                                                         |
| 全形式対応が必要な場合    | **ONLYOFFICE Document Server**（AGPL-3.0）または **Collabora Online**（MPL-2.0）がサーバー構築コストと引き換えに最適。ただし AGPL-3.0 はコード公開義務があるため商用利用時は商用ライセンスの検討が必要 |
| クラウド利用可能な場合    | ファイルを公開 URL で配信できるなら **Microsoft Office Online** の iframe 埋め込みが最高忠実度                                                                                                         |
| プリザンター統合          | 認証付き添付ファイルの制約とライセンスの両面から、パーミッシブライセンスのクライアントサイドライブラリ（docx-preview + SheetJS）の組み合わせが最も現実的                                               |
