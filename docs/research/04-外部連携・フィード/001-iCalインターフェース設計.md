...
ビュー・フィルタ適用、複数設定の管理方法、VEVENT の SUMMARY/DESCRIPTION スタイルを整理する。
...
- [VEVENT の SUMMARY・DESCRIPTION スタイル](#vevent-の-summarydescription-スタイル)
    - [標準スタイル](#標準スタイル)
    - [カスタムデザインスタイル](#カスタムデザインスタイル)
    - [スタイル判定フロー](#スタイル判定フロー)
- [管理画面の設計](#管理画面の設計)
...
| SUMMARY スタイル       | 標準（画面表示タイトル）またはカスタム（`[カラム名]` プレースホルダ形式のテンプレート） |
| DESCRIPTION スタイル   | 標準（Body カラム）またはカスタム（`[カラム名]` プレースホルダ形式のテンプレート）      |
...
erDiagram
    ...
    ICalSettings {
        ...
        int UserId
        int ViewId
        nvarchar16 SummaryStyle
        nvarcharmax SummaryFormat
        nvarchar16 DescriptionStyle
        nvarcharmax DescriptionFormat
        bit Disabled
...
| `SummaryStyle`      | `nvarchar(16)`  | `Standard`（画面表示タイトル）または `Custom`（テンプレート形式）  |
| `SummaryFormat`     | `nvarchar(max)` | カスタム SUMMARY テンプレート（例: `[Title] ([ClassA])`）          |
| `DescriptionStyle`  | `nvarchar(16)`  | `Standard`（Body カラム）または `Custom`（テンプレート形式）       |
| `DescriptionFormat` | `nvarchar(max)` | カスタム DESCRIPTION テンプレート（例: `[Body]\n担当: [Manager]`） |
| `Disabled`          | `bit`           | 無効フラグ                                                         |
...
public class ICalSetting : ISettingListItem
{
    ...
    public int UserId;
    public int ViewId;
    public string SummaryStyle;      // "Standard" または "Custom"
    public string SummaryFormat;     // カスタム時のテンプレート文字列
    public string DescriptionStyle;  // "Standard" または "Custom"
    public string DescriptionFormat; // カスタム時のテンプレート文字列
...
## VEVENT の SUMMARY・DESCRIPTION スタイル

VEVENT に出力する `SUMMARY`（件名）と `DESCRIPTION`（説明）について、
**標準** と **カスタムデザイン** の 2 パターンを設定できるようにする。

### 標準スタイル

| フィールド    | 出力値                        | 取得元                              |
| ------------- | ----------------------------- | ----------------------------------- |
| `SUMMARY`     | 画面表示タイトル（ItemTitle） | `Items.Title` を JOIN で取得する値  |
| `DESCRIPTION` | 内容（Body）                  | `Results.Body` または `Issues.Body` |

**ItemTitle** は Pleasanter のカレンダービューで通常表示されるタイトルと同じ値で、
`Items` テーブルの `Title` カラムを JOIN した `ItemTitle` カラムから取得する。

**ファイル**: `Implem.Pleasanter/Libraries/DataSources/Rds.cs`（行番号: 817-829）

```csharp
// Items テーブルを JOIN して ItemTitle を取得
column.ItemTitle(ss.ReferenceType)
// → Items_Items."Title" AS "ItemTitle"
```

**実装上の注意**: CalendarDataRows の既存 SQL は `Body` を取得しないため、
標準スタイルの DESCRIPTION を生成するには `Body` カラムを SELECT に追加する必要がある。

**ファイル**: `Implem.Pleasanter/Models/Results/ResultUtilities.cs`（行番号: 8696-8714）

```csharp
// CalendarDataRows に Body 取得を追加する例
column: Rds.ResultsTitleColumn(context: context, ss: ss)
    .ResultId(_as: "Id")
    .SiteId(_as: "SiteId")
    .Body()        // ← 追加（標準 DESCRIPTION 用）
    .ResultsColumn(columnName: fromColumn.ColumnName, _as: "From")
    ...
...
**SearchFormat**（`Link.cs`）や **GridDesign**（`Column.cs`）と同じ
`[カラム名]` プレースホルダ形式のテンプレート文字列で指定する。

| フィールド    | 設定例                                            | 説明                                            |
| ------------- | ------------------------------------------------- | ----------------------------------------------- |
| `SUMMARY`     | `[Title] ([Manager])`                             | SearchFormat と同様の `[ColumnName]` 形式で指定 |
...
```csharp
// TdCustomValue の置換ロジック（iCal SUMMARY / DESCRIPTION に転用）
ss.IncludedColumns(format).ForEach(column =>
{
    var value = GetColumnValue(context, ss, column, resultModel); // カラム別に値を取得
    format = format.Replace("[" + column.ColumnName + "]", value);
});
// format がプレースホルダ置換後の最終テキストになる
```

**ファイル**: `Implem.Pleasanter/Libraries/Settings/SiteSettings.cs`（行番号: 4426-4447）

```csharp
// IncludedColumns でテンプレートに含まれるカラムを抽出
...
`SummaryFormat`・`DescriptionFormat` に含まれるカラム名を
`SiteSettings.IncludedColumns()` で抽出し、CalendarDataRows の SQL に追加して一括取得する。

```csharp
// SummaryFormat + DescriptionFormat に含まれるカラムを追加取得する例
var extraColumns = ss.IncludedColumns(setting.SummaryFormat ?? "")
    .Concat(ss.IncludedColumns(setting.DescriptionFormat ?? ""))
    .Distinct();
...
```mermaid
flowchart TD
    A["SUMMARY 生成"] --> B{"SummaryStyle == 'Custom'?"}
    B -->|Yes| C["SummaryFormat の [ColumnName] を置換"]
    B -->|No| D["ItemTitle（画面表示タイトル）をそのまま使用"]

    E["DESCRIPTION 生成"] --> F{"DescriptionStyle == 'Custom'?"}
    F -->|Yes| G["DescriptionFormat の [ColumnName] を置換"]
...
| SUMMARY スタイル         | enum   | `Standard`（ItemTitle）または `Custom`                    |
| SUMMARY フォーマット     | string | カスタム時のテンプレート（例: `[Title] ([ClassA])`）      |
| DESCRIPTION スタイル     | enum   | `Standard`（Body）または `Custom`                         |
| DESCRIPTION フォーマット | string | カスタム時のテンプレート（例: `[Body]\n担当: [Manager]`） |
...
2. 実行ユーザ・ビューを選択
3. SUMMARY スタイル（Standard / Custom）を選択し、Custom の場合はフォーマット文字列を入力
4. DESCRIPTION スタイル（Standard / Custom）を選択し、Custom の場合はフォーマット文字列を入力
5. 「保存」→ `ICalSettings` テーブルに INSERT/UPDATE
6. URL をクリップボードコピーしてカレンダーアプリに登録
...
sequenceDiagram
    ...
    DB_IC-->>IC: SiteId, TenantId, UserId, ViewId,<br/>SummaryStyle, SummaryFormat,<br/>DescriptionStyle, DescriptionFormat
    ...
    IC->>DB_R: SELECT ItemTitle, Body(*), From, To,<br/>カスタムフォーマット内カラム(*) WHERE view.Where()
    Note right of DB_R: (*) Standard/Custom スタイルに応じて追加取得
    DB_R-->>IC: DataRows

    IC->>IC: ToLocal(context) で日時をユーザTZ変換
    IC->>IC: EditorFormat == "Ymd" → 終日判定
    IC->>IC: SUMMARY 生成（Standard: ItemTitle / Custom: テンプレート置換）
    IC->>IC: DESCRIPTION 生成（Standard: Body / Custom: テンプレート置換）
    IC->>IC: VEVENT 生成（DTSTART, DTEND, SUMMARY, DESCRIPTION, UID）
...
| Standard の Body 取得      | CalendarDataRows の既存 SQL は `Body` を SELECT しないため追加が必要              |
| カスタムフォーマットの SQL | `IncludedColumns()` で抽出したカラムを SELECT に動的追加して一括取得する          |
| HTML タグの除去            | Body や GridDesign 系カラムは HTML を含む場合があり、DESCRIPTION 出力前に除去推奨 |
...
| SUMMARY スタイル     | Standard: `ItemTitle`（画面表示タイトル）/ Custom: `[ColumnName]` テンプレート置換   |
| DESCRIPTION スタイル | Standard: `Body` カラム / Custom: `[ColumnName]` テンプレート置換（GridDesign 方式） |
...
| `Implem.Pleasanter/Libraries/Settings/SiteSettings.cs`                        | Views・各種 SettingList・IncludedColumns の格納   |
...
| `Implem.Pleasanter/Libraries/Settings/Link.cs`                                | SearchFormat の `[ColumnName]` 置換ロジック       |
...
| `Implem.Pleasanter/Libraries/DataTypes/TitleBody.cs`                          | TitleBody（Title + Body の組合せ型）              |
| `Implem.Pleasanter/Models/Results/ResultUtilities.cs`                         | CalendarDataRows（データ取得 SQL）・TdCustomValue |
