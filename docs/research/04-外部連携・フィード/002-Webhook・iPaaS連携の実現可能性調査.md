...
- [フローエディタ UI の技術選定](#フローエディタ-ui-の技術選定)
    ...
    - [Drawflow の評価](#drawflow-の評価)
- [Drawflow を用いた実装設計](#drawflow-を用いた実装設計)
    - [前提: プリザンターのフロントエンド構成](#前提-プリザンターのフロントエンド構成)
    - [ファイル配置](#ファイル配置)
    - [ライブラリの読み込み方式](#ライブラリの読み込み方式)
    - [SiteSettings タブへの統合](#sitesettings-タブへの統合)
    - [Drawflow 初期化コード](#drawflow-初期化コード)
    - [ノードタイプの定義](#ノードタイプの定義)
    - [Drawflow ノードの HTML テンプレート例](#drawflow-ノードの-html-テンプレート例)
    - [フロー定義の保存形式](#フロー定義の保存形式)
    - [SiteSettings への保存方式](#sitesettings-への保存方式)
    - [実装ステップ](#実装ステップ)
    - [Drawflow のイベントを活用したバリデーション](#drawflow-のイベントを活用したバリデーション)
...
プリザンターには**Webhook 受信用の専用エンドポイントは存在しない**。
外部サービスの Webhook は送信元固有のペイロード形式で送信されるため、
プリザンターの REST API に直接送信することはできない。
現状では、iPaaS 等の中間層でペイロードをプリザンター API 形式に変換したうえで
REST API を呼び出す必要がある。

```mermaid
flowchart LR
    subgraph Current["現行のデータ流入経路"]
        EXT["外部サービス\n(固有ペイロード)"] -->|"Webhook"| MID["中間層\n(iPaaS等)"] -->|"REST API\n(プリザンター形式に変換)"| PL["プリザンター"]
    end
...
| データマッピング   | 中間層でプリザンター形式に変換が必要 | サーバー側でペイロードを変換する    |
...
| ライブラリ        | 特徴                                                   | ライセンス | フレームワーク依存 |
| ----------------- | ------------------------------------------------------ | ---------- | :----------------: |
| React Flow        | React ベースのノードエディタ。カスタムノード対応が柔軟 | MIT        |       React        |
| Rete.js           | フレームワーク非依存のノードエディタ。プラグイン拡張型 | MIT        |        なし        |
| Flume             | React ベース。ノードタイプ定義が宣言的で型安全         | MIT        |       React        |
| **Drawflow**      | **軽量（~11KB）。バニラ JS。依存なし**                 | **MIT**    |      **なし**      |
| Node-RED エディタ | Node-RED のエディタコンポーネントを埋め込み利用する    | Apache-2.0 |     jQuery/D3      |
...
プリザンターのフロントエンドアーキテクチャとの適合性を評価した結果、
**Drawflow が最も適したライブラリ**であると判断した。

#### Drawflow の概要

[Drawflow](https://github.com/jerosoler/Drawflow) はシンプルなフロー
エディタライブラリで、バニラ JavaScript で動作し外部依存がない。

| 項目                 | 内容                                              |
| -------------------- | ------------------------------------------------- |
| バンドルサイズ       | ~11KB（minified + gzip）                          |
| 依存ライブラリ       | なし（バニラ JS）                                 |
| ライセンス           | MIT                                               |
| フレームワーク対応   | バニラ JS / Vue 2 / Vue 3                         |
| ノード定義           | HTML 文字列またはテンプレートで自由に定義可能     |
| 入出力ポート         | ノードごとに任意の数の入力/出力ポートを設定可能   |
| データバインディング | `df-*` 属性で input/textarea/select と自動同期    |
| モジュール           | フローをモジュール単位に分割して管理可能          |
| エディタモード       | `edit`（編集） / `fixed`（固定） / `view`（閲覧） |
| ズーム               | Ctrl + マウスホイール / ピンチ操作に対応          |
| モバイル対応         | タッチ操作（ドラッグ・ピンチ）に対応              |
| エクスポート形式     | JSON（フロー全体をシリアライズ可能）              |

#### プリザンターとの適合性

| 評価項目            | 評価 | 理由                                                                     |
| ------------------- | :--: | ------------------------------------------------------------------------ |
| jQuery 3.6 との共存 |  適  | バニラ JS のため jQuery と競合しない                                     |
| Svelte との共存     |  適  | DOM 操作が独立しておりコンポーネント間の干渉がない                       |
| 導入コスト          |  低  | JS + CSS 各1ファイルの追加のみ。ビルドパイプライン変更不要               |
| 学習コスト          |  低  | API がシンプル（4行で初期化可能）                                        |
| カスタムノード      |  適  | HTML 文字列で定義するため、プリザンターの HtmlBuilder パターンと親和性高 |
| JSON 永続化         |  適  | export() / import() メソッドで JSON 化でき、SiteSettings に保存可能      |
| 拡張ライブラリ配置  |  適  | `wwwroot/Extensions/` に配置する既存パターンに従える                     |

#### 他ライブラリとの比較

| 比較項目           | Drawflow    | React Flow           | Rete.js        | Node-RED エディタ   |
| ------------------ | ----------- | -------------------- | -------------- | ------------------- |
| バンドルサイズ     | ~11KB       | ~50KB + React        | ~30KB          | ~500KB + 依存多数   |
| jQuery との親和性  | 競合なし    | React 仮想DOM競合    | 競合なし       | jQuery/D3 依存      |
| 初期化の容易さ     | 4行         | React セットアップ   | プラグイン構成 | Node-RED 全体が必要 |
| カスタムノード定義 | HTML 文字列 | React コンポーネント | プラグイン     | 専用ノード形式      |
| JSON 永続化        | 標準対応    | 別途実装             | 標準対応       | 標準対応            |
| ビルド依存         | 不要        | 必要                 | 不要           | 必要                |
...
プリザンターのフロントエンドは以下の構成で動作している。

| 要素            | 技術                  | 説明                                            |
| --------------- | --------------------- | ----------------------------------------------- |
| メインフレーム  | jQuery 3.6.0          | DOM 操作・イベント処理・Ajax の基盤             |
| UI ウィジェット | jQuery UI             | ダイアログ・タブ・ドラッグ可能な要素            |
| コンポーネント  | Svelte（Vite ビルド） | 一部の新しいUIコンポーネント                    |
| HTML 生成       | C# HtmlBuilder        | サーバーサイドで HTML を生成し ViewBag で配信   |
| 拡張スクリプト  | ExtendedScript        | Extensions テーブルまたはパラメータ経由でJS注入 |
| 拡張スタイル    | ExtendedStyle         | 同上で CSS 注入                                 |
| 静的ライブラリ  | `wwwroot/Extensions/` | mermaid 等の外部ライブラリ配置先                |

### ファイル配置

Drawflow のファイルは既存の `wwwroot/Extensions/` パターンに従って配置する。

```text
Implem.Pleasanter/wwwroot/Extensions/
  drawflow.min.js       -- Drawflow ライブラリ本体
  drawflow.min.css      -- Drawflow 基本スタイル
  mermaid-11.9.0.min.js -- 既存: Mermaid ライブラリ
  smt-json-to-table.html -- 既存
```

### ライブラリの読み込み方式

Drawflow の読み込みは、プリザンターの既存の拡張スクリプト機構を利用する。
サイト設定画面でのみ読み込む方法を2つ示す。

#### 方式 A: ExtendedScript / ExtendedStyle で注入

Extensions テーブルにスクリプトとスタイルを登録し、
`action: "Edit"` かつ `controller: "Items"` のフィルタ条件で
サイト設定画面のみに Drawflow を読み込む。

#### 方式 B: HtmlScripts.cs で静的に読み込む

`HtmlScripts.cs` に Drawflow のスクリプトタグ生成を追加する。
サイト設定画面でのみ読み込む条件分岐を入れる。

```csharp
// HtmlScripts.cs への追加例
.Script(
    src: Responses.Locations.Get(
        context: context,
        parts: "Extensions/drawflow.min.js"),
    nonce: context.Nonce,
    _using: context.Action == "SiteSettings")
```

方式 A は本体改修不要で導入できる利点があるが、
方式 B の方がパフォーマンスと管理面で優れている。

### SiteSettings タブへの統合

プリザンターのサイト設定画面は `SiteUtilities.cs` の
`SettingsEditor` メソッドでタブを生成している。
フローエディタを新しいタブとして追加する。

```mermaid
flowchart LR
    subgraph SiteSettings["サイト設定画面"]
        direction TB
        T1["一覧"] --- T2["フィルタ"] --- T3["集計"]
        T3 --- T4["エディタ"] --- T5["リンク"] --- T6["履歴"]
        T6 --- T7["通知"] --- T8["リマインダ"]
        T8 --- NEW["フロー（新規タブ）"]
    end

    NEW --> FE["Drawflow エディタ"]
    FE --> SAVE["JSON export → SiteSettings 保存"]
```
...
既存のタブ定義パターンに従い、`#FlowSettingsEditor` タブを追加する。

```csharp
// 既存パターン
.Li(action: () => hb
    .A(href: "#NotificationsSettingsEditor",
       text: Displays.Notifications(context: context)))
// 追加するフロータブ
.Li(action: () => hb
    .A(href: "#FlowSettingsEditor",
       text: Displays.Flow(context: context)))
```

### Drawflow 初期化コード

サイト設定画面の「フロー」タブが選択されたときに
Drawflow エディタを初期化する。

```javascript
// フローエディタ初期化（ExtendedScript または専用 JS ファイル）
(function () {
    ...
    // 既存フロー定義の読み込み
    var savedFlow = $('#FlowDefinition').val();
    if (savedFlow) {
        try {
            editor.import(JSON.parse(savedFlow));
        } catch (e) {
    ...
    // ノードタイプの登録
    editor.registerNode('trigger', createTriggerNode());
    editor.registerNode('action', createActionNode());
    editor.registerNode('condition', createConditionNode());
    editor.registerNode('webhook', createWebhookNode());

    // 保存時にフロー定義を hidden フィールドに書き出す
    $('#FlowSettingsEditor').on('save', function () {
        var data = editor.export();
        $('#FlowDefinition').val(JSON.stringify(data));
    });
})();
```
...
iPaaS フローで必要となるノードタイプを定義する。

```mermaid
flowchart LR
    subgraph Triggers["トリガーノード"]
        T1["Webhook 受信"]
        T2["スケジュール"]
        T3["レコード変更"]
    end
    subgraph Actions["アクションノード"]
        A1["API 呼び出し"]
        A2["レコード操作"]
        A3["通知送信"]
        A4["スクリプト実行"]
...
| ノードタイプ   | 入力 | 出力 | パラメータ                             |
| -------------- | :--: | :--: | -------------------------------------- |
| Webhook 受信   |  0   |  1   | WebhookId, 検証方式                    |
| スケジュール   |  0   |  1   | Cron 式, タイムゾーン                  |
| レコード変更   |  0   |  1   | SiteId, イベント種別（作成/更新/削除） |
| API 呼び出し   |  1   |  1   | URL, メソッド, ヘッダー, ボディ        |
| レコード操作   |  1   |  1   | SiteId, 操作種別, フィールドマッピング |
| 通知送信       |  1   |  0   | 通知先, テンプレート                   |
| スクリプト実行 |  1   |  1   | サーバースクリプト本文                 |
| 条件分岐       |  1   |  2+  | 条件式（JavaScript）                   |
| ループ         |  1   |  1   | 繰り返し条件                           |
| 待機           |  1   |  1   | 待機時間（秒）                         |

### Drawflow ノードの HTML テンプレート例

各ノードは HTML テンプレートとして定義し、`df-*` 属性でデータバインディングする。

```html
<!-- Webhook 受信トリガーノード -->
<div class="node-content trigger-node">
    <div class="node-header">Webhook 受信</div>
    <div class="node-body">
        <label>Webhook ID</label>
        <input type="text" df-webhookId />
        <label>検証方式</label>
        <select df-verification>
...
<div class="node-content action-node">
    <div class="node-header">レコード操作</div>
    <div class="node-body">
        <label>サイト ID</label>
        <input type="number" df-siteId />
        <label>操作</label>
        <select df-operation>
...
Drawflow の `export()` メソッドが出力する JSON をそのまま
SiteSettings に保存する。
        ...
        "Home": {
            "data": {
                "1": {
                    "id": 1,
                    "name": "webhook",
                    "data": { "webhookId": "abc-123", "verification": "hmac" },
                    "class": "trigger-node",
                    "html": "...",
                    "inputs": {},
                    "outputs": {
                ...
                "2": {
                    ...
                    "data": { "siteId": 12345, "operation": "create" },
                    "class": "action-node",
                    "html": "...",
                    "inputs": {
                        "input_1": {
...
| 保存先候補                     | 利点                                    | 課題                            |
| ------------------------------ | --------------------------------------- | ------------------------------- |
| `SiteSettings.FlowDefinition`  | SiteSettings の既存パターンに統一できる | JSON サイズが大きくなる可能性   |
| Extensions テーブル            | サイト設定と分離して管理できる          | 参照時に追加クエリが必要        |
| 専用テーブル `FlowDefinitions` | フロー実行履歴と紐付けやすい            | テーブル追加の CodeDefiner 対応 |

SiteSettings に `FlowDefinition` プロパティを追加する方式が最も自然である。

```csharp
// SiteSettings.cs への追加
...
```mermaid
flowchart TB
    S1["Step 1: Drawflow 配置\nwwwroot/Extensions/ に\nJS/CSS を配置"]
    S2["Step 2: 読み込み設定\nHtmlScripts.cs に\n条件付きスクリプトタグ追加"]
    S3["Step 3: タブ追加\nSiteUtilities.cs に\nフロータブを追加"]
    S4["Step 4: SiteSettings 拡張\nFlowDefinition\nプロパティ追加"]
    S5["Step 5: 初期化スクリプト\nDrawflow エディタの\n初期化・保存ロジック"]
    S6["Step 6: ノード定義\n各ノードタイプの\nHTMLテンプレート作成"]
    S7["Step 7: フロー実行エンジン\nBackgroundServerScript ベースの\nフロー実行ランタイム"]
...
| ステップ | 規模 | 説明                                                   |
| :------: | :--: | ------------------------------------------------------ |
|    1     |  小  | Drawflow の JS/CSS を `wwwroot/Extensions/` に配置     |
|    2     |  小  | `HtmlScripts.cs` にスクリプトタグを条件付き追加        |
|    3     |  小  | `SiteUtilities.cs` にフロータブを追加                  |
|    4     |  小  | `SiteSettings.cs` に `FlowDefinition` プロパティ追加   |
|    5     |  中  | Drawflow エディタの初期化・保存・読み込みロジック      |
|    6     |  中  | 各ノードタイプの HTML テンプレートとバリデーション     |
|    7     |  大  | フロー定義 JSON を解釈して実行するバックエンドエンジン |

Step 1-4 はプリザンター本体への最小限の改修であり、
既存のコードパターンに従って実装できる。
Step 5-6 は Drawflow API を活用したフロントエンド開発、
Step 7 はバックエンド側のフロー実行エンジン開発となる。

### Drawflow のイベントを活用したバリデーション

Drawflow のイベントシステムを活用し、
不正なフロー構成を検出するバリデーションを実装できる。

```javascript
// 接続作成時のバリデーション
editor.on('connectionCreated', function (connection) {
    var outputNode = editor.getNodeFromId(connection.output_id);
    var inputNode = editor.getNodeFromId(connection.input_id);
    // トリガーノードへの入力接続を禁止
    if (inputNode.class.includes('trigger-node')) {
        ...
        alert('トリガーノードに入力接続はできません');
    }
});

// ノード削除時の確認
editor.on('nodeRemoved', function (id) {
    console.log('ノード削除: ' + id);
});
```
...
| Webhook 受信（Inbound）  | 専用エンドポイントは存在しない。REST API で代替可能だが、外部サービスの Webhook ペイロードは固定形式のため受信側でプリザンター形式への変換が必要になる      |
...
| フローエディタ UI        | **Drawflow が最適**。バニラ JS・依存なし・~11KB と軽量で、jQuery ベースのプリザンター UI と競合せず統合できる。`wwwroot/Extensions/` への配置のみで導入可能 |
