...
- [代替アプローチ](#代替アプローチ)
    - [案A: パラメータのレイヤー分離（Override 方式）](#案a-パラメータのレイヤー分離override-方式)
    - [案B: 3-way マージ（デフォルト差分方式）](#案b-3-way-マージデフォルト差分方式)
    - [案C: デフォルト同梱 + 差分検出ツール](#案c-デフォルト同梱--差分検出ツール)
    - [案D: Override 方式 + 3-way マージ（ハイブリッド）](#案d-override-方式--3-way-マージハイブリッド)
    - [案E: C# デフォルト値 + 部分 JSON 方式](#案e-c-デフォルト値--部分-json-方式)
    - [代替案の比較](#代替案の比較)
- [結論](#結論)
- [関連ソースコード](#関連ソースコード)
...
### 案D: Override 方式 + 3-way マージ（ハイブリッド）

案A と案B を組み合わせる。通常運用は Override 方式で、バージョンアップ時に 3-way マージで Override ファイルの互換性を自動検証・修正する。
...
| 推奨案           | **案E（C# デフォルト値 + 部分 JSON 方式）** が最もシンプルかつ実現性が高い。既存のデシリアライズ機構と `required: false` パターンを活用し、C# クラスにデフォルト値を整備するだけで実現できる |
