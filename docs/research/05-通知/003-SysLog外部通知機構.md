...
- [方式 6: DB トリガーによる外部通知](#方式-6-db-トリガーによる外部通知)
    ...
    - [DB トリガーの非同期処理](#db-トリガーの非同期処理)
- [方式 7: 外部ログ監視ツールによる通知](#方式-7-外部ログ監視ツールによる通知)
    - [SysLogs テーブルのデータ量とパフォーマンス特性](#syslogs-テーブルのデータ量とパフォーマンス特性)
...
トリガーは INSERT トランザクション内で同期実行されるため、トリガー内のエラーや遅延が
SysLog 書き込みに直接影響する。各 RDBMS で非同期化の方法が異なる。

#### SQL Server: sp_send_dbmail は内部的に非同期

`sp_send_dbmail` は Service Broker のキューにメッセージを投入するだけで、
実際の SMTP 送信はバックグラウンドの Database Mail プロセスが行う。
そのため、トリガー内の `sp_send_dbmail` 呼び出し自体は高速に完了する。

```mermaid
sequenceDiagram
    participant App as プリザンター
    participant DB as SysLogs テーブル
    participant Trg as INSERT トリガー
    participant Q as Service Broker Queue<br/>(msdb)
    participant BG as Database Mail<br/>バックグラウンド
    participant SMTP as SMTP サーバー

    App->>DB: INSERT INTO SysLogs
    DB->>Trg: AFTER INSERT 起動
    Trg->>Q: sp_send_dbmail<br/>(キュー投入のみ・高速)
    Q-->>Trg: 即座に返却
    Trg-->>DB: トリガー完了
    DB-->>App: INSERT 完了
    Note over BG: 非同期処理
...
この方式では INSERT への遅延は最小限（キュー投入のみ）であり、
SMTP サーバーの応答遅延やタイムアウトが INSERT をブロックすることはない。
ただし、Service Broker キューの蓄積やエラー時のリトライには注意が必要。

#### PostgreSQL: pg_notify はトランザクションコミット時に配信

`pg_notify()` はトランザクションのコミット時に通知を配信する。
トリガー関数内の `pg_notify()` 呼び出し自体はほぼゼロコストであり、
通知ペイロードの構築のみが INSERT トランザクションに加算される。
...
sequenceDiagram
    ...
    participant DB as SysLogs テーブル
    participant Trg as INSERT トリガー
    participant Ch as NOTIFY チャネル
    participant W as 外部ワーカー

    App->>DB: INSERT INTO SysLogs
    DB->>Trg: AFTER INSERT 起動
    Trg->>Trg: pg_notify() 呼び出し<br/>(ペイロード構築のみ)
    Trg-->>DB: トリガー完了
    DB-->>App: COMMIT 時に通知配信
    DB->>Ch: NOTIFY 発行
    Ch->>W: 非同期受信
    W->>W: Slack/メール送信
```

注意点として、pg_notify のペイロードサイズは最大 8,000 バイトに制限されている。
ErrStackTrace を含めるとこの制限を超える可能性があるため、
ペイロードには SysLogId のみを含め、外部ワーカー側で詳細を SELECT する方式が安全。
...
MySQL のトリガーには非同期処理の仕組みがない。
中間テーブルへの INSERT は軽量だが、トリガー内のエラーは SysLog INSERT に影響する。

#### RDBMS 別トリガーのパフォーマンス特性

| RDBMS      | トリガー内処理      | INSERT への遅延 | エラー時の影響            |
| ---------- | ------------------- | --------------- | ------------------------- |
| SQL Server | キュー投入のみ      | 極小（< 1ms）   | キュー投入失敗時のみ      |
| PostgreSQL | ペイロード構築のみ  | 極小（< 1ms）   | 構築失敗時のみ            |
| MySQL      | 中間テーブル INSERT | 小（数 ms）     | INSERT 失敗でロールバック |
...
### SysLogs テーブルのデータ量とパフォーマンス特性

外部監視方式（特に方式 7b の DB ポーリング）のパフォーマンスを評価するには、
SysLogs テーブルのデータ量を把握する必要がある。

#### 書き込み頻度

プリザンターは全 HTTP リクエストに対して SysLog を記録する。
コントローラのアクションメソッドごとに `new SysLogModel(context)` でリクエスト開始時に INSERT し、
`log.Finish(context)` でリクエスト完了時に UPDATE する。

**ファイル**: `Implem.Pleasanter/Models/SysLogs/SysLogModel.cs`

```csharp
// コンストラクタで INSERT（リクエスト開始）
public SysLogModel(Context context)
{
    Class = context.Controller;
    Method = context.Action;
    WriteSysLog(context: context, sysLogType: SysLogTypes.Info);
}

// Finish で UPDATE（リクエスト完了）
public void Finish(Context context, int responseSize = 0, ...)
{
    EndTime = DateTime.Now;
    Elapsed = (EndTime - StartTime).TotalMilliseconds;
    Update(context: context, writeSqlToDebugLog: false);
}
```

1 リクエストあたり 1 INSERT + 1 UPDATE = 2 DB 操作が発生する。

#### 推定データ量

| 条件               | 値                                      |
| ------------------ | --------------------------------------- |
| 保持期間           | 90 日（`RetentionPeriod` デフォルト値） |
| 1 日のリクエスト数 | 例: 10,000 件（中規模利用）             |
| 90 日分の行数      | 約 900,000 行                           |
| 大規模利用時       | 1 日 100,000 件の場合、約 9,000,000 行  |

#### インデックス構造

SysLogs テーブルのプライマリキーは `(CreatedTime, SysLogId)` の複合キーであり、
SysLogType には個別のインデックスが存在しない。

**ファイル**: `App_Data/Definitions/Definition_Column/SysLogs_CreatedTime.json`
...
この構造は `CreatedTime` による範囲検索には効率的だが、
`SysLogType` のみの条件検索ではフルスキャンまたは広範囲のインデックススキャンが発生する。

#### 定期削除

`DeleteSysLogsTimer`（Quartz スケジューラ）が `RetentionPeriod` 超過分を物理削除する。
大量データ時のロック競合を避けるため、`DeleteSysLogsChunkSize` でチャンク削除が可能。

**ファイル**: `Implem.Pleasanter/Models/SysLogs/SysLogUtilities.cs`

```csharp
// チャンク削除（DeleteSysLogsChunkSize > 0 の場合）
        ...
        statements: Rds.PhysicalDeleteSysLogs(
            where: Rds.SysLogsWhere()
                .CreatedTime(
                    ...
                    _operator: "<")
                .SysLogId_Between(begin: i, end: i + chunkSize - 1)));
}
```
...
| 項目                    | 影響度 | 内容                                                                 |
| ----------------------- | ------ | -------------------------------------------------------------------- |
| プリザンターへの影響    | なし   | ファイル出力は NLog が非同期で処理。アプリケーション性能に影響しない |
| ディスク I/O            | 低     | NLog の asyncWrapper でバッファリング済み。順次書き込みで高速        |
| ログファイルサイズ      | 中     | 全 SysLogType が出力される。日次ローテーションで管理                 |
| 監視ツールの CPU/メモリ | 低     | tail 方式は差分読み取りのみ。大量ファイルでも軽量                    |
...
SysLogs テーブルは大量データを保持するため、ポーリングクエリの設計が重要となる。

非効率なクエリ（SysLogType のみの条件、インデックスなし）:

```sql
-- 非推奨: SysLogType にインデックスがないためフルスキャンが発生する
SELECT * FROM "SysLogs"
WHERE "SysLogType" >= 80;
```

推奨クエリ（CreatedTime でプライマリキーのインデックスを活用）:

```sql
-- 推奨: CreatedTime 条件でクラスタードインデックスの範囲スキャンに限定
SELECT "SysLogId", "SysLogType", "Class", "Method",
       "ErrMessage", "Url", "MachineName", "CreatedTime"
FROM "SysLogs"
WHERE "CreatedTime" > @LastCheckedTime
  AND "SysLogType" >= 80
ORDER BY "CreatedTime";
```

この書き方であれば、`CreatedTime` のプライマリキーインデックスを使用して
直近のポーリング間隔分（例: 1 分間）の行のみをスキャンし、
その中で `SysLogType >= 80` をフィルタする。スキャン範囲は数百行以下に限定される。

#### 専用インデックスの追加（任意）

ポーリング間隔が長い場合やスキャン行数が多い場合は、
専用の複合インデックスを追加することで更に効率化できる。

```sql
-- SQL Server
CREATE NONCLUSTERED INDEX IX_SysLogs_Alert
ON SysLogs (CreatedTime, SysLogType)
WHERE SysLogType >= 80;

-- PostgreSQL
CREATE INDEX ix_syslogs_alert
ON "SysLogs" ("CreatedTime")
WHERE "SysLogType" >= 80;
...
SQL Server のフィルター付きインデックスと PostgreSQL の部分インデックスは
SysLogType >= 80 の行のみを含むため、インデックスサイズが極めて小さく、
INSERT 時のオーバーヘッドもほぼない（該当行が全体の 0.1% 未満のため）。

#### ポーリング間隔とパフォーマンスの関係

| ポーリング間隔 | 1 回あたりのスキャン行数 | DB 負荷 | 検知遅延   |
| -------------- | ------------------------ | ------- | ---------- |
| 10 秒          | 約 1-5 行（10K req/日）  | 極小    | 最大 10 秒 |
| 30 秒          | 約 3-15 行               | 極小    | 最大 30 秒 |
| 1 分           | 約 7-30 行               | 極小    | 最大 1 分  |
| 5 分           | 約 35-150 行             | 低      | 最大 5 分  |

CreatedTime 条件で直近の短い時間範囲のみをスキャンするため、
テーブル全体の行数（数十万〜数百万行）に関わらず、
1 回あたりのスキャン行数は一定かつ少量に抑えられる。

#### 方式 7b のパフォーマンス影響

| 項目                        | 影響度 | 内容                                                               |
| --------------------------- | ------ | ------------------------------------------------------------------ |
| プリザンターへの影響        | 極小   | 読み取りクエリのみ。書き込みロックとは競合しない                   |
| DB サーバー CPU             | 極小   | CreatedTime 範囲スキャンは数百行以下。実行計画はインデックスシーク |
| DB サーバー I/O             | 極小   | クラスタードインデックスの末尾数ページのみ読み取り                 |
| ロック競合                  | なし   | SELECT のみのため排他ロックは取得しない                            |
| DeleteSysLogsTimer との競合 | なし   | 削除対象は RetentionPeriod（90 日前）、ポーリングは直近数分間      |
...
| 方式                          | コード変更 | 設定変更       | リアルタイム性 | バージョンアップ追従 | フィルタ柔軟性 | プリザンター非汚染 | パフォーマンス影響 |
| ----------------------------- | ---------- | -------------- | -------------- | -------------------- | -------------- | ------------------ | ------------------ |
| 1. NLog ターゲット追加        | 不要       | appsettings 等 | 即時           | 影響なし             | 高             | -                  | なし               |
| 2. 既存通知基盤流用           | 大規模     | 不可           | 即時           | 高コスト             | 中             | -                  | 中                 |
| 3. バックグラウンドポーリング | 必要       | 不可           | 遅延あり       | 中コスト             | 高             | -                  | 低                 |
| 4. サーバースクリプト         | 不要       | 部分的         | 非対応         | 影響なし             | 低             | -                  | なし               |
| 5. HealthCheck                | 必要       | 不可           | 遅延あり       | 中コスト             | 低             | -                  | 低                 |
| 6. DB トリガー                | 不要       | DB 側のみ      | 即時           | 影響小               | 高             | 完全               | 極小（非同期）     |
| 7a. 外部ログファイル監視      | 不要       | SysLog.json    | 準即時         | 影響なし             | 高             | ほぼ完全           | なし               |
| 7b. 外部 DB ポーリング        | 不要       | 不要           | 遅延あり       | 影響なし             | 高             | 完全               | 極小（読取のみ）   |
