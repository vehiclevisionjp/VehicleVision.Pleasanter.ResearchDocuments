...
| モード              | データ取得方法       | 用途                                    |
| ------------------- | -------------------- | --------------------------------------- |
| **JSON 定義モード** | JSON 定義に基づく    | 宣言的な統合ビュー定義（標準）          |
| **拡張 SQL モード** | ExtendedSql に基づく | 複雑な SQL が必要なケース（上級者向け） |
...
flowchart LR
    ...
    B -->|ExtendedSql| D[拡張 SQL モード<br/>ExtendedSql.CommandText<br/>結果に統合 FilterHash]
    C --> E[統合一覧表示]
    D --> E
```
...
統合ビューの一覧としてそのまま表示する。
結合後の統合フィルタ・ソート（`IntegratedFilterHash` /
`IntegratedSorterHash`）は JSON 定義モードと同様に
既存のヘッダ・フィルター領域 UI から適用可能だが、
ソース別の 2 段階フィルタは適用されない
（SQL 側で WHERE / ORDER BY を自由に記述するため）。
...
flowchart TB
    ...
    subgraph 統合フィルタ・ソート
        C[IntegratedFilterHash<br/>IntegratedSorterHash<br/>ヘッダ・フィルター領域UIから操作]
    end
...
| `IntegratedFilterHash` | `Dictionary` | 統合フィルタ（結果に対して適用）             |
| `IntegratedSorterHash` | `Dictionary` | 統合ソート（結果に対して適用）               |

**排他制御**:

- `DataSourceType = "Json"` の場合: `Sources` / `CombineType` /
  `JoinCondition` が有効。`ExtendedSqlName` は無視される。
- `DataSourceType = "ExtendedSql"` の場合: `ExtendedSqlName` が有効。
  `Sources` / `CombineType` / `JoinCondition` は無視される。
- いずれのモードでも `IntegratedFilterHash` / `IntegratedSorterHash` /
  `Limit` は共通で使用可能。
...
flowchart LR
    subgraph 設定画面
        ...
        B -->|Json| C[JSON定義タブ<br/>Sources / CombineType<br/>ソース別FilterHash]
        B -->|ExtendedSql| D[拡張SQLタブ<br/>ExtendedSqlName選択]
    end

    subgraph 共通設定
        E[IntegratedFilterHash<br/>IntegratedSorterHash<br/>Limit]
    end
...
| 統合フィルタ         | ○ ヘッダ・フィルター領域 UI        | ○ ヘッダ・フィルター領域 UI    |
| 統合ソート           | ○ ヘッダ・フィルター領域 UI        | ○ ヘッダ・フィルター領域 UI    |
...
| `IntegratedFilterHash`       | `Dictionary` | **統合フィルタ**（結合後に適用、両モード共通） |
| `IntegratedSorterHash`       | `Dictionary` | **統合ソート**（結合後に適用、両モード共通）   |
...
public class IntegratedView
{
    ...
    public Dictionary<string, string> IntegratedFilterHash;   // 統合フィルタ（共通）
    public Dictionary<string, SqlOrderBy.Types> IntegratedSorterHash; // 統合ソート（共通）
    public int? Limit;
...
sequenceDiagram
    ...
    ESql-->>IV: CommandText
    Note over IV: 権限フィルタはSQL内で管理<br/>（ソース別FilterHashは適用されない）

    Note over IV: 統合FilterHash<br/>+ 統合SorterHash 適用<br/>（ヘッダ・フィルター領域UIから操作）
    IV->>DB: Repository.ExecuteDataSet()
    DB-->>IV: DataSet
