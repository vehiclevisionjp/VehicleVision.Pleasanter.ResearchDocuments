...
複数のテーブル（Issues / Results）を横断して統合表示する「統合ビュー」機能の設計を行う。簡易的な SQL ライクな SELECT 構文により、複数テーブルのデータを一覧表示し、各行から編集画面へのリンク・編集モーダルを提供する。
...
| SQL ライク SELECT 構文   | SELECT / FROM / WHERE / ORDER BY を簡易的に記述できるクエリ構文を提供する           |
...
統合ビューは新しい SiteType として実装するのではなく、**サーバスクリプトと拡張 SQL の組み合わせ**を基盤として、既存の GridData / View アーキテクチャを拡張する形で設計する。

```mermaid
flowchart TB
    subgraph ユーザ入力
        A[統合ビュー定義<br/>SQL ライク構文]
    end
...
統合ビューでは以下の SQL ライク構文をサポートする。プリザンター内部のカラム名（`IssueId`, `ResultId`, `ClassA`-`ClassZ`, `NumA`-`NumZ`, `Status` 等）を使用する。
...
| WHERE     | フィルタ条件。`=`, `<>`, `<`, `>`, `<=`, `>=`, `LIKE`, `IN`, `IS NULL` 対応 | `WHERE Status <> 900`              |
| ORDER BY  | ソート指定。`ASC`（昇順）/ `DESC`（降順）                                   | `ORDER BY UpdatedTime DESC`        |
...
FROM 句で指定された各テーブルに対して、現行の `Permissions.SetPermissionsWhere()` と同じロジックで権限フィルタが自動挿入される。ユーザが WHERE 句を記述する時点では、既に権限による絞り込みが行われている前提となる。
...
classDiagram
    ...
    class IntegratedViewQuery {
        ...
        +ToSqlStatements(Context) List~SqlStatement~
    }
...
SQL ライク構文のパースは、正規表現ベースの簡易パーサで実装する。完全な SQL パーサは不要であり、サポートする構文要素を限定することで実装コストを抑える。

```csharp
public class IntegratedViewQueryParser
{
    private static readonly Regex SelectPattern =
        new(@"SELECT\s+(.+?)\s+FROM", RegexOptions.IgnoreCase | RegexOptions.Singleline);
    private static readonly Regex FromPattern =
        new(@"FROM\s+(.+?)(?:\s+WHERE|\s+ORDER\s+BY|\s+LIMIT|\s*$)",
            RegexOptions.IgnoreCase | RegexOptions.Singleline);
    private static readonly Regex WherePattern =
        new(@"WHERE\s+(.+?)(?:\s+ORDER\s+BY|\s+LIMIT|\s*$)",
            RegexOptions.IgnoreCase | RegexOptions.Singleline);
    private static readonly Regex OrderByPattern =
        new(@"ORDER\s+BY\s+(.+?)(?:\s+LIMIT|\s*$)",
            RegexOptions.IgnoreCase | RegexOptions.Singleline);
    ...
    public IntegratedViewQuery Parse(string queryText)
    {
        ...
        // UNION ALL で分割
        var unionParts = Regex.Split(queryText, @"\bUNION\s+ALL\b",
        ...
        // SELECT / FROM / WHERE / ORDER BY / LIMIT をパース
        return query;
    }
}
```
...
プリザンター内部の `SqlColumnCollection` / `SqlWhereCollection` / `SqlOrderByCollection`
を経由して SQL を構築する。

```mermaid
flowchart LR
    A[ユーザ入力<br/>SQL ライク構文] --> B[パーサ<br/>構文分解]
    B --> C{バリデーション}
    C -->|不正| D[エラー返却]
    C -->|正常| E[SqlColumnCollection<br/>SqlWhereCollection<br/>SqlOrderByCollection]
    E --> F[パラメータ化クエリ生成]
    F --> G[DB 実行]
...
| 対策項目                 | 方法                                                               |
| ------------------------ | ------------------------------------------------------------------ |
| SQL インジェクション防止 | パーサでトークン分解後、`SqlParam` でパラメータバインド            |
...
```csharp
public List<SqlStatement> ToUnionSqlStatements(Context context)
{
    var statements = new List<SqlStatement>();
    foreach (var part in UnionParts)
    {
        ...
        // ユーザ指定 WHERE 追加
        ApplyUserWhere(context, ss, part.Conditions, where);
...
| 学習コスト   | △ SQL 知識必要      | ◎ GUI 操作              | ○ SQL 構文 + GUI             |
...
3. **段階的実装**: 最初は UNION 形式のみ実装し、後から JOIN 形式を追加するなど段階的な拡張が可能
4. **編集モーダルとの相性**: ダッシュボードには既に各パーツからの遷移・モーダル表示の基盤があり、統合ビューの編集モーダルも同じ仕組みで実装可能
...
| `Implem.Pleasanter/Libraries/Settings/IntegratedViewQuery.cs`（新規） | クエリパーサ                                   |
...
| 考慮点                   | 対策                                                           |
| ------------------------ | -------------------------------------------------------------- |
| View フィルタとの併用    | 統合ビューの WHERE 句と View フィルタは AND 条件で結合         |
...
| ------------------------ | ----------------------------------------------------------------------------------- |
| 推奨方式                 | 方式 C: ダッシュボードパーツ拡張                                                    |
| クエリ構文               | SQL ライクな SELECT / FROM / WHERE / ORDER BY / UNION ALL / JOIN をサポート         |
...
| SQL インジェクション対策 | パーサでトークン分解 → パラメータバインド。生 SQL の直接実行は禁止                  |
| 段階的実装               | Phase 1: UNION ALL 形式の統合一覧 → Phase 2: JOIN 形式 → Phase 3: エクスポート・API |
