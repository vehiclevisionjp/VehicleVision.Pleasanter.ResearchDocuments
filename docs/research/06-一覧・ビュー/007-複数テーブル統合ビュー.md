...
`Newtonsoft.Json`（プリザンター標準の JSON ライブラリ）による
JSON デシリアライズで行う。
...
public class IntegratedViewDefinitionParser
{
    ...
    {
        return JsonConvert.DeserializeObject<IntegratedView>(json);
    }
}
```
...
統合ビューは複数テーブルを横断してデータを取得・表示するため、
通常の単一テーブル操作と比較してセキュリティリスクが増大する。
以下の観点で懸念点と対策を整理する。

#### 1. SQL インジェクション

**懸念**: JSON 定義内の値（SiteId、カラム名、フィルタ値、JoinCondition 等）が
SQL 生成に使用されるため、不正な SQL が挿入されるリスクがある。

**対策**:

| 対策項目           | 方法                                                                                         |
| ------------------ | -------------------------------------------------------------------------------------------- |
| パラメータ化クエリ | JSON の値は既存 Rds レイヤ（`SqlColumnCollection` / `SqlWhereCollection`）経由でパラメータ化 |
| 生 SQL 禁止        | JSON 定義からの生 SQL 直接実行は禁止。すべて Rds レイヤのビルダ経由で SQL を生成             |
| JoinCondition 検証 | カラム名参照のみ許可。関数呼び出し・サブクエリ・セミコロン・コメント構文を拒否               |
| フィルタ値の型検証 | `ColumnFilterHash` の値を `View.Where()` 経由で処理し、型安全なパラメータに変換              |
| 拡張 SQL モード    | `ExtendedSql` の既存パラメータ化機構に依存。管理者のみ設定可能とする                         |

```mermaid
flowchart LR
    A[管理者入力<br/>JSON 定義] --> B[Newtonsoft.Json<br/>デシリアライズ]
    B --> C{バリデーション<br/>型・値・構文チェック}
    C -->|不正| D[エラー返却]
    C -->|正常| E[IntegratedView オブジェクト]
    E --> F[既存 Rds レイヤ<br/>パラメータ化クエリ生成]
    F --> G[DB 実行]
...
#### 2. 権限バイパス（テナント・サイト・行レベル）

**懸念**: 統合ビューを通じて、本来アクセス権限のないテーブルやレコードの
データが閲覧・操作される可能性がある。
...
| レベル         | 懸念                                             | 対策                                                                                      |
| -------------- | ------------------------------------------------ | ----------------------------------------------------------------------------------------- |
| テナント       | 他テナントのサイトが指定される                   | `SiteSettingsUtilities.Get()` でテナント ID を検証。異テナントのサイトは拒否              |
| サイト（Read） | 閲覧権限のないサイトが Sources に含まれる        | 各ソース処理時に `Permissions.Get()` で Read 権限を確認。権限なしの場合はソースをスキップ |
| 行レベル       | 行単位の閲覧制限が適用されない                   | 各ソースに `Permissions.SetPermissionsWhere()` で行レベル権限 WHERE を自動挿入            |
| 編集権限       | 編集権限のないレコードに編集リンクが表示される   | `Editable` フラグに加え、行ごとに Update 権限をチェックしてリンクの表示/非表示を制御      |
| 設定変更       | 一般ユーザが統合ビューの定義（JSON）を変更できる | 統合ビュー定義の変更はサイト管理権限（`ManageSite`）を持つユーザのみ許可                  |

```mermaid
flowchart TB
    subgraph ソース処理
        A[Sources 配列] --> B{テナント検証<br/>SiteSettingsUtilities.Get}
        B -->|異テナント| C[拒否]
        B -->|正常| D{Read 権限<br/>Permissions.Get}
        D -->|権限なし| E[スキップ]
        D -->|権限あり| F[行レベル権限WHERE<br/>Permissions.SetPermissionsWhere]
        F --> G[データ取得]
    end
```
...
| 対策項目                  | 方法                                                                              |
| ------------------------- | --------------------------------------------------------------------------------- |
| 型制約                    | `Newtonsoft.Json` のデシリアライズで厳格な型マッピング（`IntegratedView` クラス） |
| `TypeNameHandling` の制限 | `TypeNameHandling.None`（デフォルト）を維持。`Auto` / `All` への変更は禁止        |
| 入力サイズ上限            | JSON 定義のサイズを上限値（例: 64KB）でバリデーション                             |
| Sources 配列上限          | `Sources` 配列の要素数を上限値（推奨: 10）で制限                                  |
| 再帰・循環参照対策        | `ReferenceLoopHandling.Ignore` 設定および深さ上限チェック                         |
| 不明プロパティの無視      | `MissingMemberHandling.Ignore` で不明プロパティを安全に無視                       |

#### 4. データ漏洩（カラム・行レベル）

**懸念**: 統合ビューで本来表示すべきでないカラムや行のデータが
結合結果に含まれて漏洩する。
...
| 対策項目                | 方法                                                                                   |
| ----------------------- | -------------------------------------------------------------------------------------- |
| カラム参照制限          | `Columns[].Name` を各ソースの `SiteSettings.ColumnDefinitionHash` でホワイトリスト検証 |
| 非表示カラムの除外      | `SiteSettings` で非表示に設定されたカラムは統合ビューでも表示不可                      |
| Read 権限カラムフィルタ | カラム単位の閲覧権限設定がある場合は統合結果でも適用                                   |
| `_SourceSiteId` の露出  | 統合用の内部カラムはフロントエンドで非表示（`data-*` 属性等で保持）                    |

#### 5. 拡張 SQL モード固有のリスク

**懸念**: 拡張 SQL モードでは管理者が記述した SQL が直接実行されるため、
JSON 定義モードに比べてリスクが高い。

**対策**:

| 対策項目               | 方法                                                                    |
| ---------------------- | ----------------------------------------------------------------------- |
| 設定権限の厳格化       | 拡張 SQL モードの設定はサーバ管理者（ファイルアクセス権限）のみ         |
| ExtendedSql の既存対策 | `ExtendedSql` の既存パラメータ化・実行権限機構をそのまま活用            |
| 権限フィルタの手動管理 | 拡張 SQL モードでは権限フィルタは自動挿入されない旨をドキュメントで明記 |
| 監査ログ               | 拡張 SQL モードでのクエリ実行を監査ログに記録                           |

#### 6. CSRF / XSS

**懸念**: 統合ビューの操作（フィルタ変更・編集モーダル等）で
CSRF や XSS 攻撃を受ける可能性がある。

**対策**:

| 対策項目         | 方法                                                                      |
| ---------------- | ------------------------------------------------------------------------- |
| CSRF             | 既存の AntiForgeryToken 機構をそのまま適用（新規 SiteType でも自動適用）  |
| XSS（出力時）    | 統合結果の表示は既存の `HtmlBuilder` 経由でエスケープ。生 HTML 出力は禁止 |
| XSS（JSON 定義） | JSON 定義内の文字列値を SQL 生成前にサニタイズ。スクリプト注入を防止      |
| Content-Type     | API レスポンスの Content-Type を `application/json` に固定                |
...
flowchart TB
    subgraph 入力バリデーション
        A1[JSON サイズ上限チェック]
        A2[Sources 配列数上限チェック]
        A3[Newtonsoft.Json 厳格デシリアライズ<br/>TypeNameHandling.None]
    end

    subgraph 権限チェック
        B1[テナント ID 検証]
        B2[サイト Read 権限チェック]
        B3[行レベル権限 WHERE 自動挿入]
        B4[カラム閲覧権限フィルタ]
        B5[編集権限チェック]
    end
    ...
    subgraph クエリ生成
        C1[Rds レイヤ パラメータ化]
        C2[JoinCondition 構文検証]
        C3[カラム名ホワイトリスト検証]
    end

    subgraph 出力保護
        D1[HtmlBuilder エスケープ]
        D2[AntiForgeryToken]
    end
