...
JSON 定義を補完する参照表記として、SQL ライク構文も利用可能とする。
内部では JSON 定義に変換されて処理される。
...
-- JOIN 形式
SELECT a.IssueId AS Id, a.Title AS IssueTitle,
       b.Title AS ResultTitle, a.Status
FROM 100 AS a
JOIN 200 AS b ON a.ClassA = b.ResultId
```

> **SQL ライク構文の位置づけ:**
> JSON 定義が正式な設定形式であり、SQL ライク構文は
> 管理者向けの補助的な入力・表示手段として提供する。
> SQL 構文からの入力は内部で JSON に変換して保存する。
...
flowchart LR
    subgraph ソース別設定画面
        A1[ソースA フィルタUI] --> B1[Sources.0.ColumnFilterHash]
        A2[ソースA ソートUI] --> B2[Sources.0.ColumnSorterHash]
        A3[ソースB フィルタUI] --> B3[Sources.1.ColumnFilterHash]
        A4[ソースB ソートUI] --> B4[Sources.1.ColumnSorterHash]
    end

    subgraph 統合設定画面
        C1[統合フィルタUI] --> D1[IntegratedFilterHash]
        C2[統合ソートUI] --> D2[IntegratedSorterHash]
    end
...
| ------------------- | -------------------------- | ---------------------------- |
| フィルタ設定        | ソース設定画面の入力欄     | 統合一覧のカラムヘッダ入力欄 |
| フィルタ検索種別    | 完全一致/部分一致 等       | 完全一致/部分一致 等         |
| ソート設定          | ソース設定画面のソート指定 | カラムヘッダクリック         |
| フィルタ状態の保持  | JSON 定義に永続化          | View セッション保存          |
| フィルタ表示/非表示 | —                          | FiltersDisplayType           |
...
JSON 定義が正式な設定形式であるため、主たるパース処理は
`System.Text.Json` による JSON デシリアライズとなる。
SQL ライク構文からの入力は補助的な変換処理として
正規表現ベースの簡易パーサで JSON に変換する。
...
public class IntegratedViewDefinitionParser
{
    ...
    /// <summary>SQL ライク構文から JSON 定義に変換（補助）</summary>
    public IntegratedView ParseSql(string sqlLikeText)
    {
        var view = new IntegratedView();
        // UNION ALL で分割
        var unionParts = Regex.Split(sqlLikeText,
            @"\bUNION\s+ALL\b", RegexOptions.IgnoreCase);
        if (unionParts.Length > 1)
        {
            view.CombineType = "UnionAll";
            // 各パートの SELECT / FROM をパースして
            // Sources に変換
        }
        // SELECT / FROM / LIMIT をパース
        // ColumnFilterHash / ColumnSorterHash は
        // JSON 定義側で設定（SQL 構文には含まない）
        return view;
    }
}
```
...
| 定義形式     | JSON 定義（SQL ライク構文は補助入力として提供） |
...
| フィルタ・ソート | △ SQL 手書き        | ○ 既存 View UI 流用     | ◎ 2 段階 FilterHash 対応     |
...
3. **2 段階フィルタ・ソート**: ソース別と統合の 2 段階で既存の `ColumnFilterHash` / `ColumnSorterHash` を適用でき、きめ細かいデータ絞り込みが可能
4. **JSON 定義**: 宣言的な JSON 形式で定義を管理でき、API 経由の設定変更や版管理が容易
5. **段階的実装**: 最初は UNION 形式のみ実装し、後から JOIN 形式を追加するなど段階的な拡張が可能
6. **編集モーダルとの相性**: ダッシュボードには既に各パーツからの遷移・モーダル表示の基盤があり、統合ビューの編集モーダルも同じ仕組みで実装可能
...
| `Implem.Pleasanter/Libraries/Settings/IntegratedViewDefinitionParser.cs`（新規） | JSON パーサ + SQL ライク構文変換               |
...
sequenceDiagram
    ...
    Dashboard->>Parser: JSON 定義パース
    Parser-->>Dashboard: IntegratedView
    ...
    loop Sources 配列の各ソース
        ...
        Note over Dashboard: Phase 1: 権限WHERE<br/>+ ソース別FilterHash<br/>+ ソース別SorterHash
    end

    Note over Dashboard: Phase 2: UNION ALL / JOIN 構築
    Note over Dashboard: Phase 3: 統合FilterHash<br/>+ 統合SorterHash 適用
...
| 考慮点                   | 対策                                                                             |
| ------------------------ | -------------------------------------------------------------------------------- |
| View フィルタ・ソート    | 2 段階で ColumnFilterHash / ColumnSorterHash を利用。既存 View UI をそのまま流用 |
...
| フィルタ・ソート         | 2 段階: ソース別 + 統合の ColumnFilterHash / ColumnSorterHash（既存 View UI 流用） |
