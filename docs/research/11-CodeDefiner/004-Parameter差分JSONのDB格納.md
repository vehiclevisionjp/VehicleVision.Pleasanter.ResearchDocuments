...
- [テーブル設計](#テーブル設計)
    ...
    - [TenantId = 0 が既存テナントと衝突しないコード上の保証](#tenantid--0-が既存テナントと衝突しないコード上の保証)
...
### TenantId = 0 が既存テナントと衝突しないコード上の保証

既存の Tenants テーブルでは `TenantId` が IDENTITY（自動採番）カラムとして定義されており、
採番の開始値（Seed）が `1` に設定されている。
このため、**通常の運用で `TenantId = 0` のテナントが作成されることはない**。

#### カラム定義: Tenants_TenantId.json

**ファイル**: `App_Data/Definitions/Definition_Column/Tenants_TenantId.json`
...
`"Identity": "1"` は自動採番を有効化し、`"Seed": "1"` は開始値を `1` に指定している。

#### DDL 生成コード

CodeDefiner は上記のカラム定義から RDBMS ごとに以下の DDL を生成する。

**ファイル**: `Implem.CodeDefiner/Functions/Rds/Parts/Columns.cs`（行番号: 156-158）

```csharp
if (!noIdentity && columnDefinition.Identity)
{
    commandText += factory.Sqls.GenerateIdentity.Params(
        columnDefinition.Seed == 0 ? 1 : columnDefinition.Seed);
}
```

`Seed == 0` の場合でも `1` にフォールバックするため、
IDENTITY の開始値が `0` になることはコード上あり得ない。

各 RDBMS の `GenerateIdentity` テンプレートは以下の通りである。

**ファイル**: `Rds/Implem.SqlServer/SqlServerSqls.cs`（行番号: 42）

```csharp
public string GenerateIdentity { get; } = " identity({0}, 1)";
// Tenants の場合: identity(1, 1) → 開始値 1、増分 1
```

**ファイル**: `Rds/Implem.PostgreSql/PostgreSqlSqls.cs`（行番号: 42）

```csharp
public string GenerateIdentity { get; } =
    " generated by default as identity (start with {0} increment by 1)";
// Tenants の場合: start with 1 increment by 1
```

**ファイル**: `Implem.CodeDefiner/Functions/Rds/Parts/MySql/MySqlConstraints.cs`
（行番号: 45-46, 57）
...
// seed の算出: o.Seed == 0 ? 1 : o.Seed → 常に 1 以上
```

#### RDBMS 別の生成結果

| RDBMS      | 生成される DDL                                                                               | 採番開始値 |
| ---------- | -------------------------------------------------------------------------------------------- | :--------: |
| SQL Server | `[TenantId] int not null identity(1, 1)`                                                     |   **1**    |
| PostgreSQL | `"TenantId" integer not null generated by default as identity (start with 1 increment by 1)` |   **1**    |
| MySQL      | `ALTER TABLE "Tenants" auto_increment = 1;`                                                  |   **1**    |

#### 結論

全 RDBMS で `TenantId` の自動採番は `1` から開始されるため、
`TenantId = 0` は ParameterSources テーブルの「全テナント共通設定」として
安全に予約できる。
