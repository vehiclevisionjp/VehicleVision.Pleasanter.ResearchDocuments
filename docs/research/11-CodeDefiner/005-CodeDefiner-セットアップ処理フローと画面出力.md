...
`/y` オプションが指定されていない場合、`Starter.cs` は `Console.ReadLine()` で
ユーザー入力を待機する。入力が `"y"` と一致した場合のみ処理を継続する。`/y` オプションは
この確認をスキップし、即座に `Configurator.Configure()` へ進む。
...
プリザンター標準インストール時の定義テーブルは約 50 テーブル
（`Def.TableNameCollection()` が返す数）であり、それぞれに対して最大 3 バリアント
（通常/\_deleted/\_history）の構成が行われる。そのため、最大で約 150 回の
`ConfigureTablePart()` 呼び出しが発生する。
