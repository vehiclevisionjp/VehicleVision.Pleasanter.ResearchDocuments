...
- [Pleasanter の MySQL 実装構造](#pleasanter-の-mysql-実装構造)
    ...
    - [Provider メカニズム（マネージドサービス対応）](#provider-メカニズムマネージドサービス対応)
    - [AWS Aurora への対応状況](#aws-aurora-への対応状況)
- [実装方針の比較](#実装方針の比較)
...
- [TiDB 固有の注意事項](#tidb-固有の注意事項)
    ...
    - [TiDB Cloud（マネージドサービス）での注意事項](#tidb-cloudマネージドサービスでの注意事項)
- [実装の全体像](#実装の全体像)
- [結論](#結論)
- [関連ソースコード](#関連ソースコード)
...
### Provider メカニズム（マネージドサービス対応）

`Rds.json` の `Provider` フィールドは、データベースのホスティング形態を表し、
CodeDefiner の動作を切り替える。
...
`RdsProvider == "Local"` の場合のみ実行する処理:

| 処理                               | Local | Azure / その他 |
| ---------------------------------- | :---: | :------------: |
| ユーザー作成（KillTask）           |  Yes  |       No       |
| DB 作成（RdsConfigurator）         |  Yes  |       No       |
| ユーザー作成（UsersConfigurator）  |  Yes  |       No       |
| スキーマ作成（SchemaConfigurator） |  Yes  |       No       |
| 権限付与（PrivilegeConfigurator）  |  Yes  |       No       |

マネージドサービス（Azure SQL・AWS RDS 等）では、SA 接続によるデータベース・ユーザの作成が
制限されているため、`Provider: "Azure"` に設定することでこれらをスキップし、
事前に用意したデータベース・ユーザーでセットアップを進める。

### AWS Aurora への対応状況

Pleasanter v1.5.x では AWS Aurora に関連した以下の対応が含まれている。

#### Aurora MySQL

Aurora MySQL は MySQL ワイヤプロトコル互換であるため、`Dbms: "MySQL"` + `Provider: "Local"` でそのまま動作する。
ただし、マネージドサービスのため SA レベルの操作が制限されている場合は、
`Provider: "Azure"`（または `"Local"` 以外の任意の値）に設定してスキップする運用が必要になる場合がある。

また、Aurora MySQL では接続元ホストの制約が異なるため、`MySqlConnectingHost` を適切に設定する必要がある。

#### Aurora PostgreSQL

Aurora PostgreSQL には CodeDefiner 実行時にエラーが発生する問題があり、
コミット `ce6224ed0` で修正済みである（Pleasanter v1.5.x 以降）。

主な修正内容:

- `CreateUserForPostgres.sql`: ドル引用符を `$$` から `$xxx$` に変更（Aurora での構文制限対応）
- `CreateUserForPostgres.sql`: SA ユーザーが作成したロールを Owner ユーザーへ付与（`grant "#Uid_Owner#" to "#Uid_Sa#"`）
- `CreateDatabaseForPostgres.sql`: データベース権限の明示的な付与処理を追加

Aurora PostgreSQL は `Dbms: "PostgreSQL"` + `Provider: "Local"` で動作するが、
SA ユーザーに制限がある場合は `Provider: "Azure"` を設定する。
...
### TiDB Cloud（マネージドサービス）での注意事項

TiDB Cloud は PingCAP が提供するフルマネージド TiDB サービスであり、
Aurora MySQL と同様に SA レベルの操作が制限されている。

| 設定項目                | セルフホスト TiDB      | TiDB Cloud                    |
| ----------------------- | ---------------------- | ----------------------------- |
| `Provider`              | `"Local"`              | `"Local"` 以外（例: `"AWS"`） |
| ユーザー・DB の事前作成 | CodeDefiner が自動作成 | 事前に手動作成が必要          |
| `MySqlConnectingHost`   | `localhost` または IP  | `%`（任意ホストから接続）     |

現在の実装では `Provider` に `"Azure"` または `"Local"` のみ対応しているため、
TiDB Cloud のようなマネージドサービスで CodeDefiner を使用する場合は、
`Provider: "Azure"` を設定することでユーザー・DB 作成をスキップし、
事前に準備したデータベースへのセットアップが可能である。

将来的には `"AWS"` や `"Cloud"` のような Provider 値を追加し、
TiDB Cloud・Aurora MySQL・AWS RDS 等を一律に扱えるよう拡張することも検討できる。
...
```mermaid
flowchart TD
    A["Rds.json\nDbms: 'TiDB'\nProvider: 'Local' or 'Azure'"] --> B["RdsFactory.Create('TiDB')"]
    B --> C["TiDBObjectFactory\n（新規: MySqlObjectFactory をベース）"]
    C --> D["MySqlConnector ライブラリ\n（再利用: TiDB は MySQL 互換）"]

    A --> P{"Provider == 'Local'?"}
    P -->|Yes| Q["ユーザ・DB・スキーマ作成\n権限付与 を自動実行"]
    P -->|No| R["テーブル作成のみ実行\n（TiDB Cloud 等のマネージドサービス向け）"]
...
| Provider（マネージド）   | TiDB Cloud 等では `Provider: "Azure"` を設定し、DB/ユーザー作成処理をスキップ   |
| AWS Aurora MySQL         | `Dbms: "MySQL"` + `Provider: "Local"` または `"Azure"` で対応可能               |
| AWS Aurora PostgreSQL    | `Dbms: "PostgreSQL"` で対応可能（Pleasanter v1.5.x で Aurora 向け fix 済み）    |
...
| `Implem.DefinitionAccessor/Initializer.cs`                                    | Provider 判定と RdsProvider 設定            |
| `Implem.CodeDefiner/Functions/Rds/Configurator.cs`                            | Provider による処理分岐（Local/マネージド） |
...
| `App_Data/Definitions/Sqls/PostgreSQL/CreateUserForPostgres.sql`              | Aurora PostgreSQL 向け fix 済み             |
| `App_Data/Definitions/Sqls/PostgreSQL/CreateDatabaseForPostgres.sql`          | Aurora PostgreSQL 向け fix 済み             |
