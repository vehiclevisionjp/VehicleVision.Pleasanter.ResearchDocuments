# SQL Serverの行サイズ概算・現状診断

Pleasanterの項目拡張時にSQL Serverの8,060バイト制限を評価するため、固定長・可変長・管理領域の見積もり方と、
PowerShellによる行サイズ概算・読み取り専用の現状診断をまとめる。
...
- [SQL Serverの行サイズ簡易計算](#sql-serverの行サイズ簡易計算)
    - [計算式と出力](#計算式と出力)
    - [分類・説明・添付ファイルの見積もり](#分類説明添付ファイルの見積もり)
    - [Pleasanterで使う際の確認](#pleasanterで使う際の確認)
...
| ------------- | ------------------------------------------ | ---------------------------------------- | --------------------------------------------- | --------- | ------------------------------------------------------ |
| 2026年9月25日 | VehicleVision.Pleasanter.ResearchDocuments | copilot/sqlserver-item-expansion-methods | PowerShell 7以降／診断対象SQL Server 2016以降 | `1a03396` | 同梱ツールの仕様整理。実SQL Serverへの接続検証は未実施 |

## 調査目的

項目拡張に伴う行内サイズの増加を見積もり、現在の列定義・保存データ・行外領域の使用状況を確認する方法を整理する。
項目数だけで判断できる範囲と、実DDL・データ量・実運用での検証が必要な範囲を区別する。
...
| ファイル                                                                        | 用途                                                   |
| ------------------------------------------------------------------------------- | ------------------------------------------------------ |
| [Measure-SqlServerRowSize.ps1](../../script/Measure-SqlServerRowSize.ps1)       | 項目数と想定バイト数による行サイズの簡易計算           |
| [Get-SqlServerRowDiagnostics.ps1](../../script/Get-SqlServerRowDiagnostics.ps1) | 実DBの列定義・保存データ量・物理統計の読み取り専用診断 |

---

## 結論

固定長データ・可変長データの行内部分・管理領域を分けて評価する。
項目数だけでは可変長データや行外参照を含む安全な拡張数は確定できないため、
概算ツールと実DBの診断を使い分け、最大入力時と各操作の検証を行う。
