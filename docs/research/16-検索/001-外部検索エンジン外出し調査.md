...
| エンジン         | NuGet パッケージ                | 日本語精度  | 運用コスト | ライセンス      | 推奨シナリオ                       |
| ---------------- | ------------------------------- | ----------- | ---------- | --------------- | ---------------------------------- |
...
| Meilisearch      | `Meilisearch`                   | 中          | 低         | MIT             | 小〜中規模・導入容易性優先         |
| Typesense        | `Typesense`（コミュニティ）     | 低〜中      | 低         | 要確認（GPL-3） | 日本語が重要でない中小規模         |
| Manticore Search | `manticoresearch-net`           | 中（ngram） | 低         | 要確認（GPL-2） | リソース制約のある環境             |
| Azure AI Search  | `Azure.Search.Documents`        | 高          | Azure 従量 | Azure 専用      | Azure 環境・Azure OpenAI 連携      |
| Algolia          | `Algolia.Search`                | 低〜中      | SaaS 従量  | 有料 SaaS       | フルマネージドで運用不要           |
