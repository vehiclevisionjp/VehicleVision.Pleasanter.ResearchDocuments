...
| 項目                   | 内容                                                                                                                         |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| 設定の保存方式         | 各エンティティテーブルに `SiteSettings nvarchar(max)` カラムを追加して JSON 保存（Sites と同一パターン）                     |
...
| エディタ設定           | `SiteSettings.AutoVerUpType`（バージョンアップモード）と `SiteSettings.ColumnHash[*].DefaultInput`（規定値）を設定可能にする |
...
| ファイル                                                                               | 内容                                                 |
| -------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| `Implem.Pleasanter/Models/Sites/SiteUtilities.cs`                                      | サイト設定の EditorTabs・各設定エディタメソッド      |
...
| `Implem.Pleasanter/App_Data/Definitions/Definition_Column/Sites_SiteSettings.json`     | Sites の SiteSettings カラム定義（参考）             |
| `Implem.Pleasanter/App_Data/Definitions/Definition_Column/Tenants_TenantSettings.json` | Tenants の TenantSettings カラム定義（参考）         |
