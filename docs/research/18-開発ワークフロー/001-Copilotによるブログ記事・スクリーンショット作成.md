# Copilot によるブログ記事・スクリーンショット作成ワークフロー

GitHub Copilot Coding Agent と Playwright MCP を組み合わせ、プリザンターに関するブログ記事の執筆からスクリーンショット（実行イメージ）の作成までを自動化するワークフローの実現可能性を調査する。
...
プリザンター用のブログ記事を Copilot に依頼して作成する際、記事本文だけでなくスクリーンショット（実行イメージ）まで一気通貫で生成できるかを明らかにする。

---

## 背景

ブログ記事やドキュメントを作成する際、以下のような作業が発生する。

| 作業                   | 手動の場合の負荷                                       |
| ---------------------- | ------------------------------------------------------ |
| 記事本文の執筆         | テーマの構成・下書き・推敲に時間がかかる               |
...
ブログ記事作成を Copilot に依頼する際の Issue テンプレート例を以下に示す。
...
- 画像は `docs/blog/images/` に保存すること

### 出力先

`docs/blog/YYYY-MM-DD-server-script-customization.md`
```

### Copilot Instructions での指示例

リポジトリの `.github/copilot-instructions.md` に以下のような指示を追加することで、Copilot がブログ記事作成時にスクリーンショットを自動生成するようになる。
...
| 項目              | 結論                                                                                                   |
| ----------------- | ------------------------------------------------------------------------------------------------------ |
| 実現可能性        | Copilot Coding Agent + Playwright MCP の組み合わせでブログ記事とスクリーンショットの自動生成は実現可能 |
| 推奨方式          | 方式 A（`data:` URL）を基本とし、高忠実度が必要な場合のみ方式 B を検討する                             |
| 方式 A の利点     | 外部依存なし・再現性が高い・サンドボックス環境で動作する                                               |
| 方式 B の前提条件 | セルフホストランナーまたは Codespaces + プリザンターインスタンスのネットワーク到達性が必要             |
| 運用上のポイント  | Issue テンプレートと Copilot Instructions で記事構成・画像仕様を明確に指示する                         |
