...
- [プリザンター 拡張SQL 実行権限・外部DB接続ガイド](#プリザンター-拡張sql-実行権限外部db接続ガイド)
    - [調査情報](#調査情報)
    - [調査目的](#調査目的)
    ...
    - [第4章 外部データベース接続](#第4章-外部データベース接続)
        ...
        - [4.2 SQL Server（Linked Server）](#42-sql-serverlinked-server)
            - [4.2.1 設定手順](#421-設定手順)
                - [Linked Serverの作成](#linked-serverの作成)
                - [プリザンターの接続ユーザーへの権限付与](#プリザンターの接続ユーザーへの権限付与)
                - [デフォルトマッピング（全ユーザー向け）](#デフォルトマッピング全ユーザー向け)
                - [拡張SQLの設定例](#拡張sqlの設定例)
            - [4.2.2 必要な権限](#422-必要な権限)
            - [4.2.3 ロールと権限の整理](#423-ロールと権限の整理)
            - [4.2.4 権限の調査方法](#424-権限の調査方法)
                - [Linked Serverの存在確認](#linked-serverの存在確認)
                - [現在のユーザーの権限確認](#現在のユーザーの権限確認)
                - [ログインマッピング確認](#ログインマッピング確認)
                - [接続テスト](#接続テスト)
            - [4.2.5 権限不足時のエラーと対処](#425-権限不足時のエラーと対処)
        - [4.3 PostgreSQL（Foreign Data Wrapper）](#43-postgresqlforeign-data-wrapper)
            - [4.3.1 概要](#431-概要)
            - [4.3.2 設定手順](#432-設定手順)
                - [拡張機能の有効化](#拡張機能の有効化)
                - [外部サーバーの定義](#外部サーバーの定義)
                - [ユーザーマッピングの設定](#ユーザーマッピングの設定)
                - [外部テーブルの定義](#外部テーブルの定義)
            - [4.3.3 必要な権限](#433-必要な権限)
            - [4.3.4 権限の調査方法](#434-権限の調査方法)
                - [外部サーバーの確認](#外部サーバーの確認)
                - [ユーザーマッピングの確認](#ユーザーマッピングの確認)
                - [権限確認](#権限確認)
            - [4.3.5 拡張SQLでの使用例](#435-拡張sqlでの使用例)
        - [4.4 MySQL（FEDERATEDエンジン）](#44-mysqlfederatedエンジン)
            - [4.4.1 概要](#441-概要)
            - [4.4.2 設定手順](#442-設定手順)
                - [FEDERATEDエンジンの有効化確認](#federatedエンジンの有効化確認)
                - [FEDERATEDテーブルの作成](#federatedテーブルの作成)
            - [4.4.3 必要な権限](#443-必要な権限)
            - [4.4.4 権限の調査方法](#444-権限の調査方法)
                - [FEDERATEDエンジンの有効確認](#federatedエンジンの有効確認)
                - [サーバー定義の確認](#サーバー定義の確認)
                - [FEDERATEDテーブルの確認](#federatedテーブルの確認)
            - [4.4.5 制限事項](#445-制限事項)
            - [4.4.6 拡張SQLでの使用例](#446-拡張sqlでの使用例)
    - [第5章 外部DB接続時の注意点](#第5章-外部db接続時の注意点)
        ...
        - [5.6 分散トランザクション (MSDTC) の考慮](#56-分散トランザクション-msdtc-の考慮)
            - [MSDTCが必要となるケース](#msdtcが必要となるケース)
            - [設定確認](#設定確認)
            - [MSDTC設定手順（Windows）](#msdtc設定手順windows)
            - [MSDTCを回避する方法](#msdtcを回避する方法)
    - [まとめ](#まとめ)
    - [関連ソースコード](#関連ソースコード)
    - [関連リンク](#関連リンク)
